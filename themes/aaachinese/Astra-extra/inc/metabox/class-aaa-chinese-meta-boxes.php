 ...
 * Meta Boxes setup
 */
if ( ! class_exists( 'AAA_Chinese_Meta_Boxes' ) ) {

	/**
	 * Meta Boxes setup
	 */
	class AAA_Chinese_Meta_Boxes {
		/**
		 * Instance
		 *
		 * @var $instance
		...
		public function remove_metabox() {
			...
			if ( 'fl-theme-layout' === $post_type && $post_id ) {
					remove_meta_box( 'aaa_chinese_settings_meta_box', 'fl-theme-layout', 'side' );
			}
		}
		...
		public function setup_meta_box() {
			...
			foreach ( $post_types as $type ) {
				...
				if ( 'attachment' !== $type && ! in_array( $type, $excluded_post_types, true ) ) {
					add_meta_box(
						'aaa_chinese_settings_meta_box',              // Id.
						$metabox_name,                          // Title.
		...
		public function markup_meta_box( $post ) {
			...
			wp_nonce_field( basename( __FILE__ ), 'aaa_chinese_settings_meta_box' );
			...
			do_action( 'aaa_chinese_meta_box_markup_before', $meta );
			...
			<div class="disable-section-meta-wrap components-base-control__field">
				...
				<div class="disable-section-meta">
					<?php do_action( 'aaa_chinese_meta_box_markup_disable_sections_before', $meta ); ?>

					<?php
					/** @psalm-suppress DocblockTypeContradiction */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
					if ( $show_meta_field && true === AAA_Chinese_Builder_Helper::$is_header_footer_builder_active && AAA_Chinese_Builder_Helper::is_row_empty( 'above', 'header', 'desktop' ) ) {
						/** @psalm-suppress DocblockTypeContradiction */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
						?>
					...
					<?php
					/** @psalm-suppress DocblockTypeContradiction */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
					if ( $show_meta_field && AAA_Chinese_Builder_Helper::is_row_empty( 'primary', 'header', 'desktop' ) ) {
						/** @psalm-suppress DocblockTypeContradiction */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
						?>
					...
					<?php
					/** @psalm-suppress DocblockTypeContradiction */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
					if ( $show_meta_field && true === AAA_Chinese_Builder_Helper::$is_header_footer_builder_active && AAA_Chinese_Builder_Helper::is_row_empty( 'below', 'header', 'desktop' ) ) {
						/** @psalm-suppress DocblockTypeContradiction */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
						?>
					...
					<?php
					/** @psalm-suppress DocblockTypeContradiction */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
					if ( $show_meta_field && true === AAA_Chinese_Builder_Helper::$is_header_footer_builder_active && ( AAA_Chinese_Builder_Helper::is_row_empty( 'primary', 'header', 'mobile' ) || AAA_Chinese_Builder_Helper::is_row_empty( 'above', 'header', 'mobile' ) || AAA_Chinese_Builder_Helper::is_row_empty( 'below', 'header', 'mobile' ) ) ) {
						/** @psalm-suppress DocblockTypeContradiction */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
						?>
					...
					<?php do_action( 'aaa_chinese_meta_box_markup_disable_sections_after_primary_header', $meta ); ?>
					<?php if ( $show_meta_field ) { ?>
						...
						<?php
						$ast_breadcrumbs_content = aaa_chinese_get_option( 'ast-breadcrumbs-content' );
						if ( 'disabled' !== $ast_breadcrumbs_content && 'none' !== aaa_chinese_get_option( 'breadcrumb-position' ) ) {
							?>
					<div class="ast-breadcrumbs-content-option-wrap">
						...
						if ( ( in_array( $post_type, $supported_post_types ) && true === aaa_chinese_get_option( 'ast-single-' . $post_type . '-title', class_exists( 'WooCommerce' ) && 'product' === $post_type ? false : true ) ) ) {
							if ( ! ( class_exists( 'WooCommerce' ) && absint( aaa_chinese_get_post_id() ) === wc_get_page_id( 'shop' ) ) ) {
								?>
						...
						if ( $show_meta_field && true === aaa_chinese_get_option( 'enable-related-posts' ) ) {
							?>
					...
					<?php
					$footer_adv_layout = aaa_chinese_get_option( 'footer-adv' );

					if ( $show_meta_field && ( 'disabled' !== $footer_adv_layout && ! AAA_Chinese_Builder_Helper::$is_header_footer_builder_active ) ) {
						?>
					...
					$footer_sml_layout = aaa_chinese_get_option( 'footer-sml-layout' );
					if ( 'disabled' !== $footer_sml_layout || AAA_Chinese_Builder_Helper::$is_header_footer_builder_active ) {
						?>
					...
					?>
					<?php do_action( 'aaa_chinese_meta_box_markup_disable_sections_after', $meta ); ?>
				</div>
			</div>
			<?php

			do_action( 'aaa_chinese_meta_box_markup_after', $meta );
		}
		...
		public function save_meta_box( $post_id ) {
			...
			$is_valid_nonce = isset( $_POST['aaa_chinese_settings_meta_box'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['aaa_chinese_settings_meta_box'] ) ), basename( __FILE__ ) ) ? true : false;
			...
			// Added Check if current post type is excluded from AAA Chinese meta settings.
			...
			foreach ( $post_meta as $key => $data ) {
				...
				switch ( $sanitize_filter ) {
					...
					case 'FILTER_DEFAULT':
						 ...
						 */
						$meta_value = apply_filters( 'aaa_chinese_php_default_filter_input', ! empty( $_POST[ $key ] ) ? sanitize_text_field( wp_unslash( $_POST[ $key ] ) ) : '', $key );
						/** @psalm-suppress PossiblyInvalidArgument */
						break;
		...
		public function load_scripts() {
			...
			if ( defined( 'AAA_ADVANCED_HOOKS_POST_TYPE' ) && AAA_ADVANCED_HOOKS_POST_TYPE === $post_type ) {
				return;
			}
			...
			$css_uri      = AAA_THEME_URI . '/inc/metabox/extend-metabox/css/' . $dir_name;

			wp_enqueue_style( 'astra-meta-box', $css_uri . '/metabox' . $file_prefix . '.css', array(), AAA_THEME_VERSION );

			wp_enqueue_script( 'astra-meta-settings' );
			wp_set_script_translations( 'astra-meta-settings', 'astra' );
			$aaa_chinese_ext_extension_class_exists = class_exists( 'AAA_Chinese_Ext_Extension' ) ? true : false;
			...
			$palette_css_var_prefix   = AAA_Chinese_Global_Palette::get_css_variable_prefix();
			$apply_new_default_values = aaa_chinese_button_default_padding_updated();
			...
			$reorder_color_seq     = AAA_Chinese_Dynamic_CSS::aaa_chinese_4_8_9_compatibility();

			/* Created a new array specifically designed for storing post types that don't require AAA Chinese's meta settings.*/
			$register_aaa_chinese_metabox = ! in_array( $post_type, array( 'wp_block' ), true );
			...
			wp_localize_script(
				...
				array(
					...
					'isWhiteLabelled'                => aaa_chinese_is_white_labelled(),
					...
					'page_header_edit_link'          => esc_url( admin_url( 'edit.php?post_type=aaa_chinese_adv_header' ) ),
					...
					'is_bb_themer_layout'            => ! aaa_chinese_check_is_bb_themer_layout(), // Show page header option only when bb is not activated.
					'is_addon_activated'             => defined( 'AAA_EXT_VER' ) ? true : false,
					'sticky_addon_enabled'           => $aaa_chinese_ext_extension_class_exists && AAA_Chinese_Ext_Extension::is_active( 'sticky-header' ) ? true : false,
					'register_aaa_chinese_metabox'         => apply_filters( 'aaa_chinese_settings_metabox_register', $register_aaa_chinese_metabox ),
					'is_hide_contnet_layout_sidebar' => $ast_content_layout_sidebar,
					'upgrade_pro_link'               => aaa_chinese_get_pro_url( '/pricing/', 'free-theme', 'customizer', 'upgrade' ),
					'show_upgrade_notice'            => aaa_chinese_showcase_upgrade_notices(),
					// Flag needed to check whether user is old or new, true for old user, false for new.
					'v4_1_6_migration'               => ! AAA_Chinese_Dynamic_CSS::aaa_chinese_fullwidth_sidebar_support(),
					'color_addon_enabled'            => $aaa_chinese_ext_extension_class_exists && AAA_Chinese_Ext_Extension::is_active( 'colors-and-background' ) ? true : false,
					...
					'color_palette_labels'           => AAA_Chinese_Global_Palette::get_palette_labels(),
				)
			);

			wp_enqueue_script( 'astra-metabox-cf-compatibility', AAA_THEME_URI . 'inc/assets/js/custom-fields-priority.js', array(), AAA_THEME_VERSION, false );
		}
		...
		public function get_content_layout_options() {
			...
			if ( aaa_chinese_with_third_party() || $exclude_cpt ) {
				return array(
					'default'                => __( 'Customizer Setting', 'astra' ),
		 ...
		 *
		 * @return array $aaa_chinese_header_options All header dependent toggle based page elements.
		 */
		public function get_header_disable_meta_fields() {
			$aaa_chinese_header_options = array();

			if ( AAA_Chinese_Builder_Helper::is_row_empty( 'above', 'header', 'desktop' ) ) {
				$aaa_chinese_header_options[] = array(
			...
			if ( AAA_Chinese_Builder_Helper::is_row_empty( 'primary', 'header', 'desktop' ) ) {
				$aaa_chinese_header_options[] = array(
			...
			if ( AAA_Chinese_Builder_Helper::is_row_empty( 'below', 'header', 'desktop' ) ) {
				$aaa_chinese_header_options[] = array(
			...
			if (
				AAA_Chinese_Builder_Helper::is_row_empty( 'above', 'header', 'mobile' ) ||
				AAA_Chinese_Builder_Helper::is_row_empty( 'primary', 'header', 'mobile' ) ||
				AAA_Chinese_Builder_Helper::is_row_empty( 'below', 'header', 'mobile' )
			) {
				$aaa_chinese_header_options[] = array(
			...
			return $aaa_chinese_header_options;
		}

		/**
		 * Get disable section fields.
		 *
		 * @return array $aaa_chinese_page_meta_elements All toggle based page elements.
		 */
		public function get_disable_section_fields() {

			$aaa_chinese_page_meta_elements = array(
				array(
			...
			if ( 'none' !== aaa_chinese_get_option( 'breadcrumb-position', 'none' ) ) {
				$aaa_chinese_page_meta_elements[] = array(
			...
			if ( true === aaa_chinese_get_option( 'enable-related-posts' ) ) {
				$aaa_chinese_page_meta_elements[] = array(
			...
			return $aaa_chinese_page_meta_elements;
		}
		...
		public function get_sticky_header_options() {
			$aaa_chinese_sticky_header_options     = array();
			...
			if ( defined( 'AAA_EXT_VER' ) && AAA_Chinese_Ext_Extension::is_active( 'sticky-header' ) ) {
				/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				$sticky_on_devices = aaa_chinese_get_option( 'sticky-header-on-devices' );
				switch ( $sticky_on_devices ) {
					case 'desktop':
						$sticky_above_header_condition   = AAA_Chinese_Builder_Helper::is_row_empty( 'above', 'header', 'desktop' );
						$sticky_primary_header_condition = AAA_Chinese_Builder_Helper::is_row_empty( 'primary', 'header', 'desktop' );
						$sticky_below_header_condition   = AAA_Chinese_Builder_Helper::is_row_empty( 'below', 'header', 'desktop' );
						break;
					case 'mobile':
						$sticky_above_header_condition   = AAA_Chinese_Builder_Helper::is_row_empty( 'above', 'header', 'mobile' );
						$sticky_primary_header_condition = AAA_Chinese_Builder_Helper::is_row_empty( 'primary', 'header', 'mobile' );
						$sticky_below_header_condition   = AAA_Chinese_Builder_Helper::is_row_empty( 'below', 'header', 'mobile' );
						break;
					default:
						$sticky_above_header_condition   = AAA_Chinese_Builder_Helper::is_row_empty( 'above', 'header', 'desktop' ) || AAA_Chinese_Builder_Helper::is_row_empty( 'above', 'header', 'mobile' ) ? true : false;
						$sticky_primary_header_condition = AAA_Chinese_Builder_Helper::is_row_empty( 'primary', 'header', 'desktop' ) || AAA_Chinese_Builder_Helper::is_row_empty( 'primary', 'header', 'mobile' ) ? true : false;
						$sticky_below_header_condition   = AAA_Chinese_Builder_Helper::is_row_empty( 'below', 'header', 'desktop' ) || AAA_Chinese_Builder_Helper::is_row_empty( 'below', 'header', 'mobile' ) ? true : false;
						break;
				}
			}
			...
			return $aaa_chinese_sticky_header_options;
		}
		...
		public function check_page_header_availability() {
			...
			if ( class_exists( 'AAA_Chinese_Ext_Extension' ) && ! AAA_Chinese_Ext_Extension::is_active( 'advanced-headers' ) ) {
				return false;
			}

			/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			$page_headers = AAA_Chinese_Target_Rules_Fields::get_post_selection( 'aaa_chinese_adv_header' );
			/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		...
		public function get_page_header_options() {
			...
			/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			$header_options = AAA_Chinese_Target_Rules_Fields::get_post_selection( 'aaa_chinese_adv_header' );
			/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		...
		/**
		 * Setup meta options for AAA Chinese meta settings.
		 *
		 * @since 3.7.8
		 */
		...
		/**
		 * Get post types excluded from AAA Chinese meta registration.
		 *
		 * @since 4.11.6
		 * @return array
		 */
		public static function get_excluded_meta_post_types() {
			/**
			 * Filter to exclude post types from AAA Chinese meta registration.
			 *
			 * @param array $excluded_post_types Array of post types to exclude.
			 */
			return apply_filters( 'aaa_chinese_excluded_meta_post_types', array( 'sureforms_form' ) );
		}
	}
}
 ...
 * @since 3.9.4
 */
function aaa_chinese_footer_bar_display_cb( $display_footer ) {
...
add_filter( 'aaa_chinese_footer_bar_display', 'aaa_chinese_footer_bar_display_cb', 99, 1 );

/**
 * Kicking this off by calling 'get_instance()' method
 */
AAA_Chinese_Meta_Boxes::get_instance();
