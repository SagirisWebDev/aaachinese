 ...
 * @since 4.11.3
 */
if ( ! class_exists( 'AAA_Chinese_Elementor_Editor_Settings' ) ) {
	/**
	 * Elementor Editor Settings integration class.
	 *
	 * @since 4.11.3
	 */
	class AAA_Chinese_Elementor_Editor_Settings {
		/**
		 * Class instance.
		 *
		 * @var self|null
		...
		private function init_hooks() {
			...
			// Hook into Elementor's document saving process to persist AAA Chinese-specific settings.
		...
		/**
		 * Get array of setting keys for AAA Chinese Elementor settings.
		 *
		 * @since 4.11.3
		 *
		 * @return array Array of setting keys.
		 */
		public static function get_aaa_chinese_elementor_setting_keys() {
			$settings = array(
				'site-post-title', // AAA Chinese Disable Title.
			...
			/**
			 * Filter to modify the AAA Chinese Elementor setting keys.
			 *
			 * @since 4.11.3
			 *
			 * @param array $keys Array of AAA Chinese Elementor setting keys.
			 */
			return apply_filters( 'aaa_chinese_elementor_page_setting_keys', $settings );
		}
		...
		public function enqueue_editor_assets() {
			...
			$path   = AAA_THEME_URI . '/inc/metabox/extend-metabox/build/elementor.js';

			// Get the script dependencies from the asset file.
			$dependencies = array();
			if ( file_exists( AAA_THEME_DIR . 'inc/metabox/extend-metabox/build/elementor.asset.php' ) ) {
				$assets = require AAA_THEME_DIR . 'inc/metabox/extend-metabox/build/elementor.asset.php';
			...
			/**
			 * Filter to modify the localized data for AAA Chinese astraElementor editor settings.
			 *
			 * @since 4.11.3
			 * @param array $localize_data Localized data for astraElementor editor settings.
			 */
			$localize_data = apply_filters(
				'aaa_chinese_elementor_editor_localize_data',
				array(
					'themeName'    => aaa_chinese_get_theme_name(),
					'themeIconUrl' => esc_url( apply_filters( 'aaa_chinese_admin_menu_icon', AAA_THEME_URI . 'inc/assets/images/astra-logo.svg' ) ),
		...
		public static function get_formatted_container_layout_options() {
			$options = AAA_Chinese_Meta_Boxes::get_instance()->get_content_layout_options();
		...
		public static function get_formatted_container_style_options() {
			$options = AAA_Chinese_Meta_Boxes::get_instance()->get_content_style_options();
		...
		public static function get_formatted_sidebar_layout_options() {
			$options = AAA_Chinese_Meta_Boxes::get_instance()->get_sidebar_options();
		...
		public static function get_formatted_sidebar_style_options() {
			$options = AAA_Chinese_Meta_Boxes::get_instance()->get_sidebar_style_options();
		...
		public static function get_formatted_header_enabled_options() {
			$options = AAA_Chinese_Meta_Boxes::get_instance()->get_header_enabled_options();
		 ...
		 * providing a placeholder div for the React-powered AAA Chinese Settings UI.
		 *
		...
		public function register_document_controls( $document ) {
			...
			$document->start_controls_section(
				'aaa_chinese_section',
				array(
					'label' => esc_html__( 'AAA Chinese Settings', 'astra' ),
			...
			$controls = AAA_Chinese_Meta_Boxes::get_instance()->get_disable_section_fields();
			...
			$controls = AAA_Chinese_Meta_Boxes::get_instance()->get_header_disable_meta_fields();
			foreach ( $controls as $details ) {
				$meta_key = $details['key'];
				$document->add_control(
					$meta_key,
			...
			if ( defined( 'AAA_EXT_VER' ) && class_exists( 'AAA_Chinese_Ext_Extension' ) && AAA_Chinese_Ext_Extension::is_active( 'sticky-header' ) ) {
				$document->add_control(
					'stick-header-meta',
					array(
						'classes' => 'ast-selector-control',
				...
				$controls = AAA_Chinese_Meta_Boxes::get_instance()->get_sticky_header_options();
				foreach ( $controls as $details ) {
					$meta_key = $details['key'];
					$document->add_control(
						$meta_key,
					...
					array(
						...
						'raw'  => sprintf(
							'<div class="ast-pro-upgrade-cta-wrapper">
								<img src="%1$s" alt="AAA Chinese Logo">
								<p class="elementor-control-field-description">%2$s</p>
								<a href="%3$s" class="ast-pro-upgrade-link" target="_blank">%4$s</a>
							</div>',
							esc_url( AAA_THEME_URI . 'inc/assets/images/astra-logo.svg' ),
							__( 'Unlock your full design potential and build a website to be proud of with AAA Chinese Pro.', 'astra' ),
							aaa_chinese_get_pro_url( '/pricing/', 'free-theme', 'elementor-editor', 'upgrade' ),
			...
			$document->add_control(
				...
				array(
					'type'        => \Elementor\Controls_Manager::NOTICE,
					'notice_type' => 'info',
					'content'     => __( "Changes to 'Hide Title' will automatically sync with AAA Chinese’s 'Disable Title' option.", 'astra' ),
				)
			);
		...
		/**
		 * Adds inline styles to enhance AAA Chinese controls in Elementor Page Settings.
		 *
		...
		public function add_custom_control_style() {
			...
			echo class_exists( 'AAA_Chinese_Minify' ) ? AAA_Chinese_Minify::trim_css( $css_rules ) : $css_rules;
		}

		/**
		 * Handle Elementor template redirect for previewing AAA Chinese settings changes.
		 *
		 * @since 4.11.3
		 */
		public function handle_preview() {
			...
			$keys     = self::get_aaa_chinese_elementor_setting_keys();
			...
			add_action( 'aaa_chinese_the_title_enabled', '__return_true', 999 );
		}

		/**
		 * Sync specific AAA Chinese settings from Elementor's Page Settings to post meta.
		 *
		...
		public static function get_svg( $icon = '' ) {
			if ( ! class_exists( 'AAA_Chinese_Builder_UI_Controller' ) ) {
				return '';
			}

			// Ensure the SVG icons are loaded.
			$svg_icons = AAA_Chinese_Builder_UI_Controller::$ast_svgs;
			if ( ! AAA_Chinese_Builder_UI_Controller::$ast_svgs ) {
				AAA_Chinese_Builder_UI_Controller::fetch_svg_icon();
				$svg_icons = AAA_Chinese_Builder_UI_Controller::$ast_svgs;
			}
		...
		/**
		 * Get excluded post types for AAA Chinese Meta setting in Elementor editor.
		 *
		 * @since 4.11.11
		 * @return array
		 */
		public static function get_excluded_post_types() {
			/**
			 * Filter to exclude post types from rendering AAA Chinese Meta setting in Elementor editor.
			 *
			 * @param array $excluded_post_types Array of post types to exclude.
			 */
			return apply_filters(
				'aaa_chinese_elementor_editor_excluded_post_types',
				array(
					'astra-advanced-hook', // AAA Chinese Theme Builder Advanced Layout.
 ...
 */
AAA_Chinese_Elementor_Editor_Settings::get_instance();
