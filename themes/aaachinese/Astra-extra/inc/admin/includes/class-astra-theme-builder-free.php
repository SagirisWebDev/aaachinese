...
if ( ! class_exists( 'Astra_Theme_Builder_Free' ) ) {
	...
	define( 'AAA_THEME_BUILDER_FREE_DIR', AAA_THEME_DIR . 'admin/assets/theme-builder/' );
	define( 'AAA_THEME_BUILDER_FREE_URI', AAA_THEME_URI . 'admin/assets/theme-builder/' );
	...
	class Astra_Theme_Builder_Free {
		...
		public function __construct() {
			$is_astra_addon_active = defined( 'AAA_EXT_VER' );
		...
		public function theme_builder_admin_enqueue_scripts() {
			...
			wp_enqueue_style( 'astra-theme-builder-style', AAA_THEME_BUILDER_FREE_URI . 'build/index' . $file_prefix . '.css', array(), AAA_THEME_VERSION );

			wp_enqueue_script( 'astra-theme-builder-script', AAA_THEME_BUILDER_FREE_URI . 'build/index.js', array( 'wp-element' ), AAA_THEME_VERSION, true );
		...
		public function astra_theme_builder_access_denied_redirect() {
			...
			if ( isset( $_GET['page'] ) && 'theme-builder-free' === $_GET['page'] ) {
				...
				$is_astra_addon_active = ( defined( 'AAA_EXT_VER' ) && Astra_Ext_Extension::is_active( 'advanced-hooks' ) );
				/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				if ( $is_astra_addon_active ) {
