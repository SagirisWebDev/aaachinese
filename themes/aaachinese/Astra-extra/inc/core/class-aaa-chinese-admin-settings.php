...
if ( ! class_exists( 'Astra_Admin_Settings' ) ) {
	...
	class Astra_Admin_Settings {
		...
		public static function load_astra_admin_script() {
			...
			wp_register_script( 'astra-admin-settings', AAA_THEME_URI . 'inc/assets/js/astra-admin-menu-settings.js', array( 'jquery', 'wp-util', 'updates' ), AAA_THEME_VERSION, false );
		...
		public static function register_notices() {
			...
			if ( astra_is_white_labelled() || ! AAA_THEME_ORG_VERSION || false === apply_filters( 'astra_showcase_starter_templates_notice', true ) || ! Astra_Menu::is_promoting_starter_templates() ) {
				/** @psalm-suppress RedundantCondition */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				return;
			}
			...
			if ( current_user_can( 'install_plugins' ) && ! defined( 'AAA_SITES_NAME' ) && '1' == get_option( 'fresh_site' ) && ! in_array( $current_slug, array( 'astra-advanced-hook', 'astra_adv_header' ), true ) ) {
				...
				$image_path                          = AAA_THEME_URI . 'inc/assets/images/astra-banner.png';
				$gb_image_path                       = AAA_THEME_URI . 'inc/assets/images/gb-logo.svg';
				$ele_image_path                      = AAA_THEME_URI . 'inc/assets/images/ele-logo.svg';
				$ai_image_path                       = AAA_THEME_URI . 'inc/assets/images/ai-logo.svg';
		...
		public static function upgrade_to_pro_wc_notice() {
			...
			if ( '' !== $current_slug && function_exists( 'WC' ) && in_array( $current_slug, array( 'wc-admin', 'wc-reports', 'wc-status', 'wc-addons', 'wc-settings' ), true ) ) {
				...
				$image_path = AAA_THEME_URI . 'inc/assets/images/astra-logo.svg';
		...
		public static function minimum_addon_version_notice() {
			...
			if ( version_compare( AAA_EXT_VER, AAA_EXT_MIN_VER ) < 0 ) {
				...
				if ( ! $min_version ) {
					update_user_meta( get_current_user_id(), 'ast-minimum-addon-version-notice-min-ver', AAA_EXT_MIN_VER );
				}

				if ( version_compare( $min_version, AAA_EXT_MIN_VER, '!=' ) ) {
		...
		public static function get_astra_addon_min_supported_version( $input_version ) {
			if ( defined( 'AAA_EXT_VER' ) && version_compare( AAA_EXT_VER, AAA_EXT_MIN_VER ) < 0 ) {
				return AAA_EXT_MIN_VER;
			}
		...
		public static function minimum_addon_supported_version_notice() {
			...
			// AAA_EXT_MIN_VER < AAA_EXT_VER && AAA_EXT_VER < 4.0.0.
			if ( version_compare( AAA_EXT_VER, AAA_EXT_MIN_VER ) >= 0 || version_compare( '4.0.0', AAA_EXT_VER ) < 0 ) {
				return;
			}

			$astra_addon_supported_version = self::get_astra_addon_min_supported_version( AAA_EXT_VER );
			$message                       = sprintf(
		...
		public static function notice_assets() {
			if ( is_rtl() ) {
				wp_enqueue_style( 'astra-custom-notices-rtl', AAA_THEME_URI . 'inc/assets/css/astra-notices-rtl.css', array(), AAA_THEME_VERSION );
			} else {
				wp_enqueue_style( 'astra-custom-notices', AAA_THEME_URI . 'inc/assets/css/astra-notices.css', array(), AAA_THEME_VERSION );
			}
		}
		...
		public static function get_starter_templates_slug() {
			...
			if ( defined( 'AAA_PRO_SITES_VER' ) && version_compare( AAA_PRO_SITES_VER, '2.0.0', '>=' ) ) {
				self::$starter_templates_slug = 'starter-templates';
			}

			if ( defined( 'AAA_SITES_VER' ) && version_compare( AAA_SITES_VER, '2.0.0', '>=' ) ) {
				self::$starter_templates_slug = 'starter-templates';
			}
		}
			...
			if ( in_array( $hook, array( 'post.php', 'post-new.php' ) ) ) {
				...
				if ( null !== $post_type && is_callable( 'use_block_editor_for_post_type' ) && use_block_editor_for_post_type( $post_type ) && in_array( $post_type, (array) $post_types ) ) {
					...
					wp_enqueue_script(
						...
						AAA_THEME_URI . 'inc/assets/js/column-block-compatibility.js',
