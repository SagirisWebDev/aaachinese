...
class Astra_Starter_Content {
	...
	public function get_customizer_json() {
		try {
			$request = wp_remote_get( AAA_THEME_URI . 'inc/compatibility/starter-content/astra-settings-export.json' );
		} catch ( Exception $ex ) {
			$request = null;
		}
	...
	public function get() {
		...
		$content = array(
			...
			'posts'       => array(
				self::HOME_SLUG => require AAA_THEME_DIR . 'inc/compatibility/starter-content/home.php', // PHPCS:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
			),
		);
