 ...
 * @since 4.0.0
 */
class AAA_Chinese_Posts_Single_Structures_Configs extends AAA_Chinese_Customizer_Config_Base {
	/**
	...
	public function get_new_content_layout_choices( $post_type ) {
		if ( ! in_array( $post_type, AAA_Chinese_Posts_Structures_Configs::get_narrow_width_exculde_cpts() ) ) {
			return array(
				'default'                => array(
				...
				'narrow-width-container' => array(
					...
					'path'  => class_exists( 'AAA_Chinese_Builder_UI_Controller' ) ? AAA_Chinese_Builder_UI_Controller::fetch_svg_icon( 'narrow-width-container', false ) : '',
				),
	...
	public function get_layout_configuration( $parent_section, $post_type ) {
		return array(
			 ...
			 */
			array(
				'name'              => AAA_THEME_SETTINGS . '[single-' . $post_type . '-ast-content-layout]',
				'type'              => 'control',
				'control'           => 'ast-radio-image',
				'sanitize_callback' => array( 'AAA_Chinese_Customizer_Sanitizes', 'sanitize_choices' ),
				'section'           => $parent_section,
				'default'           => aaa_chinese_get_option( 'single-' . $post_type . '-ast-content-layout', 'default' ),
				'priority'          => 3,
			 ...
			 */
			array(
				'name'        => AAA_THEME_SETTINGS . '[single-' . $post_type . '-content-style]',
				'type'        => 'control',
				'control'     => 'ast-selector',
				'section'     => $parent_section,
				'default'     => aaa_chinese_get_option( 'single-' . $post_type . '-content-style', 'default' ),
				'priority'    => 3,
			 ...
			 */
			array(
				'name'              => AAA_THEME_SETTINGS . '[single-' . $post_type . '-sidebar-layout]',
				'type'              => 'control',
				'control'           => 'ast-radio-image',
				'sanitize_callback' => array( 'AAA_Chinese_Customizer_Sanitizes', 'sanitize_choices' ),
				'section'           => $parent_section,
				'default'           => aaa_chinese_get_option( 'single-' . $post_type . '-sidebar-layout', 'default' ),
			 ...
			 */
			array(
				'name'       => AAA_THEME_SETTINGS . '[single-' . $post_type . '-sidebar-style]',
				'type'       => 'control',
				'control'    => 'ast-selector',
				'section'    => $parent_section,
				'default'    => aaa_chinese_get_option( 'single-' . $post_type . '-sidebar-style', 'default' ),
				'priority'   => 3,
			 ...
			 */
			array(
				'name'       => AAA_THEME_SETTINGS . '[page-single-width]',
				'type'       => 'control',
				'control'    => 'ast-selector',
				'section'    => $parent_section,
				'default'    => aaa_chinese_get_option( 'page-single-width' ),
				'priority'   => 6,
			 ...
			 */
			array(
				'name'        => AAA_THEME_SETTINGS . '[page-single-max-width]',
				'type'        => 'control',
				'control'     => 'ast-slider',
				'section'     => $parent_section,
				'transport'   => 'postMessage',
				'default'     => aaa_chinese_get_option( 'page-single-max-width' ),
				'context'     => array(
					AAA_Chinese_Builder_Helper::$general_tab_config,
					array(
						'setting'  => AAA_THEME_SETTINGS . '[page-single-width]',
						'operator' => '===',
						'value'    => 'custom',
					),
				),
	...
	public function register_configuration( $configurations, $wp_customize ) {
		...
		$post_types = AAA_Chinese_Posts_Structure_Loader::get_supported_post_types();
		foreach ( $post_types as $index => $post_type ) {
			...
			$default_edd_featured_image = true === aaa_chinese_enable_edd_featured_image_defaults();
			...
			$structure_defaults = aaa_chinese_get_option( $title_section . '-structure', $_structure_defaults );
			...
			if ( count( $taxonomies ) > 1 ) {
				...
				$clone_limit = 3;
				$to_clone    = true;
				if ( absint( aaa_chinese_get_option( $title_section . '-taxonomy-clone-tracker', 1 ) ) === $clone_limit ) {
					$to_clone = false;
				}
				$meta_config_options[ $title_section . '-taxonomy' ]   = array(
					...
					'clone_tracker' => AAA_THEME_SETTINGS . '[' . $title_section . '-taxonomy-clone-tracker]',
			...
			// Display Read Time option in Meta options only when AAA Chinese Addon is activated.
			...
			$_configs = array(
				...
				array(
					'name'     => AAA_THEME_SETTINGS . '[ast-single-' . $post_type . '-title]',
					'type'     => 'control',
					'default'  => aaa_chinese_get_option( 'ast-single-' . $post_type . '-title', class_exists( 'WooCommerce' ) && 'product' === $post_type ? false : true ),
				 ...
				 */
				array(
					'name'                   => AAA_THEME_SETTINGS . '[' . $title_section . '-layout]',
					'type'                   => 'control',
					'control'                => 'ast-radio-image',
					'sanitize_callback'      => array( 'AAA_Chinese_Customizer_Sanitizes', 'sanitize_choices' ),
					'section'                => $title_section,
					'default'                => aaa_chinese_get_option( $title_section . '-layout', 'layout-1' ),
					'priority'               => 5,
					...
					'choices'                => array(
						'layout-1' => array(
							...
							'path'  => AAA_Chinese_Builder_UI_Controller::fetch_svg_icon( 'banner-layout-1' ),
						),
						'layout-2' => array(
							'label' => __( 'Layout 2', 'astra' ),
							'path'  => AAA_Chinese_Builder_UI_Controller::fetch_svg_icon( 'banner-layout-2' ),
						),
					),
				 ...
				 */
				array(
					'name'       => AAA_THEME_SETTINGS . '[' . $title_section . '-banner-width-type]',
					'type'       => 'control',
					'control'    => 'ast-selector',
					'section'    => $title_section,
					'default'    => aaa_chinese_get_option( $title_section . '-banner-width-type', 'fullwidth' ),
					'priority'   => 10,
					...
					'context'    => array(
						...
						'relation' => 'AND',
						array(
							'setting'  => AAA_THEME_SETTINGS . '[' . $title_section . '-layout]',
							'operator' => '===',
							'value'    => 'layout-2',
						),
					),
				 ...
				 */
				array(
					'name'        => AAA_THEME_SETTINGS . '[' . $title_section . '-banner-custom-width]',
					'type'        => 'control',
					'control'     => 'ast-slider',
					'section'     => $title_section,
					'transport'   => 'postMessage',
					'default'     => aaa_chinese_get_option( $title_section . '-banner-custom-width', 1200 ),
					'context'     => array(
						AAA_Chinese_Builder_Helper::$general_tab_config,
						'relation' => 'AND',
						array(
						...
						array(
							'setting'  => AAA_THEME_SETTINGS . '[' . $title_section . '-banner-width-type]',
							'operator' => '===',
							'value'    => 'custom',
						),
					),
				 ...
				 */
				array(
					'name'              => AAA_THEME_SETTINGS . '[' . $title_section . '-structure]',
					'type'              => 'control',
					'control'           => 'ast-sortable',
					'sanitize_callback' => array( 'AAA_Chinese_Customizer_Sanitizes', 'sanitize_multi_choices' ),
					'section'           => $title_section,
					'context'           => AAA_Chinese_Builder_Helper::$general_tab,
				...
				array(
					...
					'parent'      => AAA_THEME_SETTINGS . '[' . $title_section . '-structure]',
					'type'        => 'sub-control',
					'control'     => 'ast-selector',
					'section'     => $title_section,
					'default'     => aaa_chinese_get_option( $title_section . '-article-featured-image-position-layout-1', 'behind' ),
					'priority'    => 28,
				...
				array(
					...
					'transport'   => 'postMessage',
					'section'     => $title_section,
					'default'     => aaa_chinese_get_option( $title_section . '-article-featured-image-position-layout-2', 'none' ),
					'priority'    => 28,
					'title'       => __( 'Image Position', 'astra' ),
					'choices'     => array(
						'none'  => __( 'None', 'astra' ),
				...
				array(
					...
					'transport'  => 'postMessage',
					'section'    => $title_section,
					'default'    => aaa_chinese_get_option( $title_section . '-article-featured-image-width-type', 'wide' ),
					'priority'   => 28,
				...
				array(
					...
					'default'                => aaa_chinese_get_option( $title_section . '-article-featured-image-ratio-type', 'predefined' ),
					'type'                   => 'sub-control',
					'section'                => $title_section,
					'parent'                 => AAA_THEME_SETTINGS . '[' . $title_section . '-structure]',
					'linked'                 => $title_section . '-image',
				...
				array(
					...
					'default'    => aaa_chinese_get_option( $title_section . '-article-featured-image-ratio-pre-scale', '16/9' ),
					'type'       => 'sub-control',
					'section'    => $title_section,
					'parent'     => AAA_THEME_SETTINGS . '[' . $title_section . '-structure]',
					'linked'     => $title_section . '-image',
				...
				array(
					...
					'default'           => aaa_chinese_get_option( $title_section . '-article-featured-image-custom-scale-width', 16 ),
					'type'              => 'sub-control',
					'control'           => 'ast-number',
					'parent'            => AAA_THEME_SETTINGS . '[' . $title_section . '-structure]',
					'linked'            => $title_section . '-image',
					...
					'sanitize_callback' => array( 'AAA_Chinese_Customizer_Sanitizes', 'sanitize_number_n_blank' ),
				),
				array(
					'name'              => $title_section . '-article-featured-image-custom-scale-height',
					'default'           => aaa_chinese_get_option( $title_section . '-article-featured-image-custom-scale-height', 9 ),
					'type'              => 'sub-control',
					'control'           => 'ast-number',
					'transport'         => 'postMessage',
					'section'           => $title_section,
				...
				array(
					...
					'default'     => aaa_chinese_get_option( $title_section . '-article-featured-image-size', 'large' ),
					'section'     => $title_section,
					'transport'   => 'postMessage',
					'type'        => 'sub-control',
					'parent'      => AAA_THEME_SETTINGS . '[' . $title_section . '-structure]',
					...
					'divider'     => array( 'ast_class' => 'ast-top-divider' ),
					'control'     => 'ast-select',
					'choices'     => aaa_chinese_get_site_image_sizes( true ),
					'description' => defined( 'AAA_EXT_VER' ) ? __( "You can specify Custom image sizes from the Single Post's 'Featured Image Size' option.", 'astra' ) : '',
				),

				array(
					'name'        => $title_section . '-remove-featured-padding',
					'parent'      => AAA_THEME_SETTINGS . '[' . $title_section . '-structure]',
					'default'     => aaa_chinese_get_option( $title_section . '-remove-featured-padding', false ),
					'linked'      => $title_section . '-image',
					'type'        => 'sub-control',
					'control'     => 'ast-toggle',
					'section'     => $title_section,
				...
				array(
					...
					'parent'    => AAA_THEME_SETTINGS . '[' . $title_section . '-structure]',
					'default'   => aaa_chinese_get_option( $title_section . '-featured-as-background', false ),
					'linked'    => $title_section . '-image',
					'type'      => 'sub-control',
					'control'   => 'ast-toggle',
					'section'   => $title_section,
				...
				array(
					...
					'parent'   => AAA_THEME_SETTINGS . '[' . $title_section . '-structure]',
					'default'  => aaa_chinese_get_option( $title_section . '-banner-featured-overlay', '' ),
					'linked'   => $title_section . '-image',
					'type'     => 'sub-control',
					'control'  => 'ast-color',
					'section'  => $title_section,
				...
				array(
					'name'      => AAA_THEME_SETTINGS . '[' . $title_section . '-taxonomy-clone-tracker]',
					'section'   => $title_section,
					'type'      => 'control',
					'control'   => 'ast-hidden',
					'priority'  => 22,
					'transport' => 'postMessage',
					'partial'   => false,
					'default'   => aaa_chinese_get_option( $title_section . '-taxonomy-clone-tracker', 1 ),
				),

				array(
					'name'      => $title_section . '-structural-taxonomy',
					'parent'    => AAA_THEME_SETTINGS . '[' . $title_section . '-structure]',
					'default'   => aaa_chinese_get_option( $title_section . '-structural-taxonomy' ),
				...
				array(
					...
					'control'    => 'ast-selector',
					'section'    => $title_section,
					'default'    => aaa_chinese_get_option( $title_section . '-structural-taxonomy-style', '' ),
					'priority'   => 2,
					'linked'     => $title_section . '-str-taxonomy',
					'transport'  => 'refresh',
				...
				array(
					'name'              => AAA_THEME_SETTINGS . '[' . $title_section . '-metadata]',
					'type'              => 'control',
					'control'           => 'ast-sortable',
					'sanitize_callback' => array( 'AAA_Chinese_Customizer_Sanitizes', 'sanitize_multi_choices' ),
					'default'           => aaa_chinese_get_option( $title_section . '-metadata', array( 'comments', 'author', 'date' ) ),
					'context'           => array(
						AAA_Chinese_Builder_Helper::$general_tab_config,
						'relation' => 'AND',
						array(
							'setting'  => AAA_THEME_SETTINGS . '[' . $title_section . '-structure]',
							'operator' => 'contains',
				...
				array(
					...
					'default'   => aaa_chinese_get_option( $title_section . '-author-prefix-label', aaa_chinese_default_strings( 'string-blog-meta-author-by', false ) ),
					'parent'    => AAA_THEME_SETTINGS . '[' . $title_section . '-metadata]',
					'linked'    => 'author',
					'type'      => 'sub-control',
				...
				array(
					...
					'parent'    => AAA_THEME_SETTINGS . '[' . $title_section . '-metadata]',
					'default'   => aaa_chinese_get_option( $title_section . '-author-avatar', false ),
					'linked'    => 'author',
					'type'      => 'sub-control',
					'control'   => 'ast-toggle',
					'section'   => $title_section,
				...
				array(
					...
					'parent'      => AAA_THEME_SETTINGS . '[' . $title_section . '-metadata]',
					'default'     => aaa_chinese_get_option( $title_section . '-author-avatar-size', 30 ),
					'linked'      => 'author',
					'type'        => 'sub-control',
					'control'     => 'ast-slider',
					'transport'   => 'postMessage',
				...
				array(
					...
					'control'    => 'ast-selector',
					'section'    => $title_section,
					'default'    => aaa_chinese_get_option( $title_section . '-meta-date-type', 'published' ),
					'priority'   => 1,
					'linked'     => 'date',
					'transport'  => 'refresh',
					'title'      => __( 'Type', 'astra' ),
				...
				array(
					...
					'default'    => aaa_chinese_get_option( $title_section . '-date-format', '' ),
					'parent'     => AAA_THEME_SETTINGS . '[' . $title_section . '-metadata]',
					'linked'     => 'date',
					'type'       => 'sub-control',
					'control'    => 'ast-select',
				...
				array(
					...
					'default'    => aaa_chinese_get_option( $title_section . '-metadata-separator', '/' ),
					'type'       => 'sub-control',
					'transport'  => 'postMessage',
					'parent'     => AAA_THEME_SETTINGS . '[' . $title_section . '-structure]',
				 ...
				 */
				array(
					'name'      => AAA_THEME_SETTINGS . '[' . $title_section . '-horizontal-alignment]',
					'default'   => aaa_chinese_get_option( $title_section . '-horizontal-alignment' ),
					'type'      => 'control',
					'control'   => 'ast-selector',
					'section'   => $title_section,
					'priority'  => 29,
				 ...
				 */
				array(
					'name'       => AAA_THEME_SETTINGS . '[' . $title_section . '-vertical-alignment]',
					'default'    => aaa_chinese_get_option( $title_section . '-vertical-alignment', 'center' ),
					'type'       => 'control',
					'control'    => 'ast-selector',
					'section'    => $title_section,
					'priority'   => 30,
				 ...
				 */
				array(
					'name'              => AAA_THEME_SETTINGS . '[' . $title_section . '-banner-height]',
					'type'              => 'control',
					'control'           => 'ast-responsive-slider',
					'section'           => $title_section,
					'transport'         => 'postMessage',
					'sanitize_callback' => array( 'AAA_Chinese_Customizer_Sanitizes', 'sanitize_responsive_slider' ),
					'default'           => aaa_chinese_get_option( $title_section . '-banner-height', AAA_Chinese_Posts_Structure_Loader::get_customizer_default( 'responsive-slider' ) ),
					'context'           => array(
				 ...
				 */
				array(
					'name'        => AAA_THEME_SETTINGS . '[' . $title_section . '-elements-gap]',
					'type'        => 'control',
					'control'     => 'ast-slider',
					'section'     => $title_section,
					'transport'   => 'postMessage',
					'default'     => aaa_chinese_get_option( $title_section . '-elements-gap', 10 ),
				 ...
				 */
				array(
					'name'      => AAA_THEME_SETTINGS . '[' . $title_section . '-banner-background]',
					'type'      => 'control',
					'default'   => aaa_chinese_get_option( $title_section . '-banner-background', AAA_Chinese_Posts_Structure_Loader::get_customizer_default( 'responsive-background' ) ),
					'section'   => $title_section,
					...
					'context'   => array(
						...
						'relation' => 'AND',
						array(
							'setting'  => AAA_THEME_SETTINGS . '[' . $title_section . '-featured-as-background]',
							'operator' => '!=',
							'value'    => true,
						),
						array(
				 ...
				 */
				array(
					'name'      => AAA_THEME_SETTINGS . '[' . $title_section . '-banner-title-color]',
					'type'      => 'control',
					'control'   => 'ast-color',
					'section'   => $title_section,
					'default'   => aaa_chinese_get_option( $title_section . '-banner-title-color' ),
					'transport' => 'postMessage',
					'priority'  => 5,
				 ...
				 */
				array(
					'name'      => AAA_THEME_SETTINGS . '[' . $title_section . '-banner-text-color]',
					'type'      => 'control',
					'control'   => 'ast-color',
					'section'   => $title_section,
					'default'   => aaa_chinese_get_option( $title_section . '-banner-text-color' ),
					'priority'  => 10,
				 ...
				 */
				array(
					'name'      => AAA_THEME_SETTINGS . '[' . $title_section . '-banner-link-color]',
					'type'      => 'control',
					'control'   => 'ast-color',
					'section'   => $title_section,
					'default'   => aaa_chinese_get_option( $title_section . '-banner-link-color' ),
					'transport' => 'postMessage',
					'priority'  => 15,
				 ...
				 */
				array(
					'name'      => AAA_THEME_SETTINGS . '[' . $title_section . '-banner-link-hover-color]',
					'type'      => 'control',
					'control'   => 'ast-color',
					'section'   => $title_section,
					'default'   => aaa_chinese_get_option( $title_section . '-banner-link-hover-color' ),
					'transport' => 'postMessage',
					'priority'  => 20,
				...
				array(
					'name'      => AAA_THEME_SETTINGS . '[' . $title_section . '-banner-title-typography-group]',
					'type'      => 'control',
					'priority'  => 25,
				...
				array(
					'name'      => AAA_THEME_SETTINGS . '[' . $title_section . '-banner-text-typography-group]',
					'type'      => 'control',
					'priority'  => 30,
					'control'   => 'ast-settings-group',
					'context'   => AAA_Chinese_Builder_Helper::$design_tab,
				...
				array(
					...
					'parent'    => AAA_THEME_SETTINGS . '[' . $title_section . '-banner-text-typography-group]',
					'section'   => $title_section,
					'type'      => 'sub-control',
					'control'   => 'ast-font',
					'font_type' => 'ast-font-family',
					'default'   => aaa_chinese_get_option( $title_section . '-text-font-family', 'inherit' ),
					'title'     => __( 'Font Family', 'astra' ),
					'connect'   => AAA_THEME_SETTINGS . '[' . $title_section . '-text-font-weight]',
				...
				array(
					...
					'default'           => aaa_chinese_get_option( $title_section . '-text-font-weight', 'inherit' ),
				...
				array(
					...
					'type'              => 'sub-control',
					'control'           => 'ast-responsive-slider',
					'default'           => aaa_chinese_get_option( $title_section . '-text-font-size', AAA_Chinese_Posts_Structure_Loader::get_customizer_default( 'font-size' ) ),
					'transport'         => 'postMessage',
				...
				array(
					...
					'default' => aaa_chinese_get_option( $title_section . '-text-font-extras', AAA_Chinese_Posts_Structure_Loader::get_customizer_default( 'font-extras' ) ),
				...
				array(
					...
					'parent'    => AAA_THEME_SETTINGS . '[' . $title_section . '-banner-title-typography-group]',
					'section'   => $title_section,
					'type'      => 'sub-control',
					'control'   => 'ast-font',
					'font_type' => 'ast-font-family',
					'default'   => aaa_chinese_get_option( $title_section . '-title-font-family', 'inherit' ),
					'title'     => __( 'Font Family', 'astra' ),
					'connect'   => AAA_THEME_SETTINGS . '[' . $title_section . '-title-font-weight]',
					'divider'   => array( 'ast_class' => 'ast-sub-bottom-divider' ),
				),
				...
				array(
					...
					'font_type'         => 'ast-font-weight',
					'sanitize_callback' => array( 'AAA_Chinese_Customizer_Sanitizes', 'sanitize_font_weight' ),
					'default'           => aaa_chinese_get_option( $title_section . '-title-font-weight', AAA_Chinese_Posts_Structure_Loader::get_customizer_default( 'title-font-weight' ) ),
					'title'             => __( 'Font Weight', 'astra' ),
				...
				array(
					...
					'type'              => 'sub-control',
					'control'           => 'ast-responsive-slider',
					'default'           => aaa_chinese_get_option( $title_section . '-title-font-size', AAA_Chinese_Posts_Structure_Loader::get_customizer_default( 'title-font-size' ) ),
					'transport'         => 'postMessage',
					'title'             => __( 'Font Size', 'astra' ),
					'sanitize_callback' => array( 'AAA_Chinese_Customizer_Sanitizes', 'sanitize_responsive_slider' ),
					'suffix'            => array( 'px', 'em', 'vw', 'rem' ),
				...
				array(
					...
					'type'    => 'sub-control',
					'control' => 'ast-font-extras',
					'default' => aaa_chinese_get_option( $title_section . '-title-font-extras', AAA_Chinese_Posts_Structure_Loader::get_customizer_default( 'font-extras' ) ),
					'title'   => __( 'Font Extras', 'astra' ),
				),

				array(
					'name'      => AAA_THEME_SETTINGS . '[' . $title_section . '-banner-meta-typography-group]',
					'type'      => 'control',
					'priority'  => 35,
					'control'   => 'ast-settings-group',
					'context'   => array(
				...
				array(
					...
					'parent'    => AAA_THEME_SETTINGS . '[' . $title_section . '-banner-meta-typography-group]',
					'section'   => $title_section,
					'type'      => 'sub-control',
					'control'   => 'ast-font',
					'font_type' => 'ast-font-family',
					'default'   => aaa_chinese_get_option( $title_section . '-meta-font-family', 'inherit' ),
					'title'     => __( 'Font Family', 'astra' ),
					'connect'   => AAA_THEME_SETTINGS . '[' . $title_section . '-meta-font-weight]',
					'divider'   => array( 'ast_class' => 'ast-sub-bottom-divider' ),
				),
				...
				array(
					...
					'font_type'         => 'ast-font-weight',
					'sanitize_callback' => array( 'AAA_Chinese_Customizer_Sanitizes', 'sanitize_font_weight' ),
					'default'           => aaa_chinese_get_option( $title_section . '-meta-font-weight', 'inherit' ),
					'title'             => __( 'Font Weight', 'astra' ),
				...
				array(
					...
					'type'              => 'sub-control',
					'control'           => 'ast-responsive-slider',
					'default'           => aaa_chinese_get_option( $title_section . '-meta-font-size', AAA_Chinese_Posts_Structure_Loader::get_customizer_default( 'font-size' ) ),
					'transport'         => 'postMessage',
					'title'             => __( 'Font Size', 'astra' ),
					'sanitize_callback' => array( 'AAA_Chinese_Customizer_Sanitizes', 'sanitize_responsive_slider' ),
					'suffix'            => array( 'px', 'em', 'vw', 'rem' ),
				...
				array(
					...
					'type'    => 'sub-control',
					'control' => 'ast-font-extras',
					'default' => aaa_chinese_get_option( $title_section . '-meta-font-extras', AAA_Chinese_Posts_Structure_Loader::get_customizer_default( 'font-extras' ) ),
					'title'   => __( 'Font Extras', 'astra' ),
				),

				array(
					'name'              => AAA_THEME_SETTINGS . '[' . $title_section . '-banner-margin]',
					'default'           => aaa_chinese_get_option( $title_section . '-banner-margin', AAA_Chinese_Posts_Structure_Loader::get_customizer_default( 'responsive-spacing' ) ),
					'type'              => 'control',
				...
				array(
					'name'              => AAA_THEME_SETTINGS . '[' . $title_section . '-banner-padding]',
					'default'           => aaa_chinese_get_option( $title_section . '-banner-padding', AAA_Chinese_Posts_Structure_Loader::get_customizer_default( 'responsive-padding' ) ),
					'type'              => 'control',
					'control'           => 'ast-responsive-spacing',
					'sanitize_callback' => array( 'AAA_Chinese_Customizer_Sanitizes', 'sanitize_responsive_spacing' ),
					'section'           => $title_section,
			...
			if ( 'page' === $post_type ) {
				 ...
				 */
				$_configs[] = array(
					'name'     => AAA_THEME_SETTINGS . '[' . $title_section . '-disable-structure-meta-on-front-page]',
					'default'  => aaa_chinese_get_option( $title_section . '-disable-structure-meta-on-front-page', false ),
					'type'     => 'control',
					'section'  => $title_section,
					'context'  => array(
						AAA_Chinese_Builder_Helper::$general_tab_config,
			...
			if ( 'post' !== $post_type && AAA_Chinese_Builder_Helper::$is_header_footer_builder_active ) {
				$class    = $post_type === 'product' ? 'ast-top-section-divider' : 'ast-top-section-spacing';
				$_configs = array_merge( $_configs, AAA_Chinese_Extended_Base_Configuration::prepare_advanced_tab( $parent_section, $class ) );
			}
			...
			if ( count( $taxonomies ) > 1 ) {
				...
				for ( $index = 1; $index <= $clone_limit; $index++ ) {
					...
					$_configs[] = array(
						...
						'parent'    => AAA_THEME_SETTINGS . '[' . $title_section . '-metadata]',
						'default'   => aaa_chinese_get_option( $title_section . '-taxonomy' . $control_suffix ),
					...
					$_configs[] = array(
						...
						'parent'     => AAA_THEME_SETTINGS . '[' . $title_section . '-metadata]',
						'default'    => aaa_chinese_get_option( $title_section . '-taxonomy' . $control_suffix . '-style', '' ),
						'linked'     => $title_section . '-taxonomy' . $control_suffix,
						'type'       => 'sub-control',
						'control'    => 'ast-selector',
						'section'    => $title_section,
	...
	public function get_dynamic_section_title( $post_type_object, $post_type ) {
		...
		return apply_filters( 'aaa_chinese_single_post_title', $title . __( ' Title Area', 'astra' ), $post_type );
	}
}
 ...
 */
new AAA_Chinese_Posts_Single_Structures_Configs();
