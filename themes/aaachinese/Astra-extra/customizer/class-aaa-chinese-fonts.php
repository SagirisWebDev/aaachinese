...
final class AAA_Chinese_Fonts {
	...
	public static function get_fonts() {
		...
		do_action( 'aaa_chinese_get_fonts' );
		return apply_filters( 'aaa_chinese_add_fonts', self::$fonts );
	}

	/**
	 * Renders the <link> tag for all fonts in the $fonts array.
	 *
	 * @since 1.0.16 Added the filter 'aaa_chinese_render_fonts' to support custom fonts.
	 * @since 1.0.0
	 * @return void
	 */
	public static function render_fonts() {

		$font_list = apply_filters( 'aaa_chinese_render_fonts', self::get_fonts() );
		...
		foreach ( $font_list as $name => $font ) {
			if ( ! empty( $name ) && ! isset( $system_fonts[ $name ] ) ) {
				...
				// Add Subset.
				$subset = apply_filters( 'aaa_chinese_font_subset', '', $name );
				if ( ! empty( $subset ) ) {
					$font_subset[] = $subset;
				}
			}
		 ...
		 * @since 3.6.0
		 */
		if ( AAA_Chinese_API_Init::get_admin_settings_option( 'self_hosted_gfonts', false ) && ! is_customize_preview() && ! is_admin() ) {
			if ( AAA_Chinese_API_Init::get_admin_settings_option( 'preload_local_fonts', false ) ) {
				aaa_chinese_load_preload_local_fonts( $google_font_url );
			}
			wp_enqueue_style( 'astra-google-fonts', aaa_chinese_get_webfont_url( $google_font_url ), array(), AAA_THEME_VERSION, 'all' );
		} else {
			wp_enqueue_style( 'astra-google-fonts', $google_font_url, array(), AAA_THEME_VERSION, 'all' );
		}
	}
	...
	public static function google_fonts_url( $fonts, $subsets = array() ) {
		...
		$fonts = apply_filters( 'aaa_chinese_google_fonts', $fonts );

		$fonts = apply_filters( 'aaa_chinese_google_fonts_selected', $fonts );
		...
		foreach ( $fonts as $font_name => $font_weight ) {
			...
			if ( ! empty( $font_weight ) ) {
				...
				$font_family = str_replace( "'", '', aaa_chinese_get_prop( $font_family, 0 ) );
		...
		if ( ! empty( $family ) ) {
			...
			$font_args['display'] = aaa_chinese_get_fonts_display_property();
