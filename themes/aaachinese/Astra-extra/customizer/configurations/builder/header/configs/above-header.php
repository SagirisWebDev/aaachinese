 ...
 */
function aaa_chinese_above_header_configuration() {
	...
	$_configs = array(
		...
		array(
			'name'              => AAA_THEME_SETTINGS . '[hba-header-height]',
			'section'           => $_section,
			'transport'         => 'postMessage',
			'default'           => aaa_chinese_get_option( 'hba-header-height' ),
			'priority'          => 30,
		...
		array(
			'name'        => AAA_THEME_SETTINGS . '[hba-header-bg-obj-responsive]',
			'type'        => 'control',
			'section'     => $_section,
			'control'     => 'ast-responsive-background',
			'transport'   => 'postMessage',
			'default'     => aaa_chinese_get_option( 'hba-header-bg-obj-responsive' ),
		...
		array(
			'name'              => AAA_THEME_SETTINGS . '[hba-header-bottom-border-color]',
			'transport'         => 'postMessage',
			'default'           => aaa_chinese_get_option( 'hba-header-bottom-border-color' ),
			'type'              => 'control',
			'control'           => 'ast-color',
			...
			'context'           => array(
				...
				array(
					'setting'  => AAA_THEME_SETTINGS . '[hba-header-separator]',
					'operator' => '>=',
					'value'    => 1,
				),
			),
		...
		array(
			'name'        => AAA_THEME_SETTINGS . '[hba-header-separator]',
			'section'     => $_section,
			'priority'    => 40,
			'transport'   => 'postMessage',
			'default'     => aaa_chinese_get_option( 'hba-header-separator' ),
...
if ( AAA_Chinese_Builder_Customizer::aaa_chinese_collect_customizer_builder_data() ) {
	add_action( 'init', 'aaa_chinese_above_header_configuration' );
}
