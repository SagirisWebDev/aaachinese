...
class Astra_Menu {
	...
	public function setup_menu() {
		...
		$show_custom_layout_submenu = defined( 'AAA_EXT_VER' ) && ! Astra_Ext_Extension::is_active( 'advanced-hooks' ) ? false : true;
		/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort

		if ( $show_custom_layout_submenu && defined( 'AAA_EXT_VER' ) && version_compare( AAA_EXT_VER, '4.5.0', '<' ) ) {
			add_submenu_page( // phpcs:ignore WPThemeReview.PluginTerritory.NoAddAdminPages.add_menu_pages_add_submenu_page -- Taken the menu on top level
				self::$plugin_slug,
				...
				defined( 'AAA_EXT_VER' ) && Astra_Ext_Extension::is_active( 'advanced-hooks' ) ? 'edit.php?post_type=astra-advanced-hook' : 'admin.php?page=' . self::$plugin_slug . '&path=custom-layouts'
			);
		}
		...
		if ( ! astra_is_white_labelled() ) {
			...
			if ( AAA_THEME_ORG_VERSION && class_exists( 'WooCommerce' ) ) {
				/** @psalm-suppress TypeDoesNotContainType */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				add_submenu_page( // phpcs:ignore WPThemeReview.PluginTerritory.NoAddAdminPages.add_menu_pages_add_submenu_page -- Taken the menu on top level
					self::$plugin_slug,
					'WooCommerce',
			...
			// elseif ( AAA_THEME_ORG_VERSION && ! $this->spectra_has_top_level_menu() ) {
			// add_submenu_page( // phpcs:ignore WPThemeReview.PluginTerritory.NoAddAdminPages.add_menu_pages_add_submenu_page -- Taken the menu on top level
			// self::$plugin_slug,
			// 'Spectra',
			// 'Spectra',
	...
	public function styles_scripts() {
		...
		wp_enqueue_style( 'astra-admin-font', 'https://fonts.googleapis.com/css2?family=Inter:wght@200;400;500&display=swap', array(), AAA_THEME_VERSION ); // Styles.

		wp_enqueue_style( 'wp-components' );

		/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		$show_self_branding = defined( 'AAA_EXT_VER' ) && is_callable( 'Astra_Ext_White_Label_Markup::show_branding' ) ? Astra_Ext_White_Label_Markup::show_branding() : true;
		/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		$user_firstname = wp_get_current_user()->user_firstname;
		/** @psalm-suppress TypeDoesNotContainType */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		$astra_addon_locale = AAA_THEME_ORG_VERSION ? 'astra-addon/astra-addon.php' : 'astra-pro/astra-pro.php';
		/** @psalm-suppress TypeDoesNotContainType */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		$localize = array(
			'current_user'            => ! empty( $user_firstname ) ? ucfirst( $user_firstname ) : ucfirst( wp_get_current_user()->display_name ),
			'admin_base_url'          => admin_url(),
			'plugin_dir'              => AAA_THEME_URI,
			'plugin_ver'              => defined( 'AAA_EXT_VER' ) ? AAA_EXT_VER : '',
			'version'                 => AAA_THEME_VERSION,
			'pro_available'           => defined( 'AAA_EXT_VER' ) ? true : false,
			'pro_installed_status'    => 'installed' === self::get_plugin_status( $astra_addon_locale ) ? true : false,
			'astra_addon_locale'      => $astra_addon_locale,
			/** @psalm-suppress TypeDoesNotContainType */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			'astra_rating_url'        => AAA_THEME_ORG_VERSION ? 'https://wordpress.org/support/theme/astra/reviews/?rate=5#new-post' : 'https://woo.com/products/astra/#reviews',
			/** @psalm-suppress TypeDoesNotContainType */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			...
			'logo_url'                => apply_filters( 'astra_admin_menu_icon', AAA_THEME_URI . 'inc/assets/images/astra-logo.svg' ),
			'update_nonce'            => wp_create_nonce( 'astra_update_admin_setting' ),
			/** @psalm-suppress TypeDoesNotContainType */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			'show_plugins'            => apply_filters( 'astra_show_free_extend_plugins', true ) && AAA_THEME_ORG_VERSION ? true : false, // Legacy filter support.
			/** @psalm-suppress TypeDoesNotContainType */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			...
			'astraWebsite'            => array(
				...
				'whatsNewFeedUrl'        => esc_url( AAA_WEBSITE_BASE_URL . '/whats-new/feed/' ),
			),
			/** @psalm-suppress TypeDoesNotContainType */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
	...
	public static function get_starter_template_plugin_data() {
		...
		$st_data = array(
			...
			'is_available' => defined( 'AAA_PRO_SITES_VER' ) || defined( 'AAA_SITES_VER' ) ? true : false,
		...
		return apply_filters(
			...
			array(
				...
				'advanced-footer'       => array(
					...
					'class'     => 'ast-addon',
					'title_url' => astra_get_pro_url( '/docs/footer-widgets-aaa-chinese-pro/', 'free-theme', 'dashboard', 'documentation' ),
					'links'     => array(
						array(
							'link_class'   => 'ast-learn-more',
							'link_url'     => astra_get_pro_url( '/docs/footer-widgets-aaa-chinese-pro/', 'free-theme', 'dashboard', 'documentation' ),
							'link_text'    => __( 'Documentation', 'astra' ),
							'target_blank' => true,
						),
					),
				...
				'advanced-hooks'        => array(
					'title'           => defined( 'AAA_EXT_VER' ) && version_compare( AAA_EXT_VER, '4.5.0', '<' ) ? __( 'Custom Layouts', 'astra' ) : __( 'Site Builder', 'astra' ),
				...
				'woocommerce'           => array(
					'title'     => 'WooCommerce',
					'class'     => 'ast-addon',
					'isActive'  => defined( 'AAA_EXT_VER' ) && class_exists( 'WooCommerce' ) ? true : false,
				...
				'edd'                   => array(
					...
					'class'     => 'ast-addon',
					'isActive'  => defined( 'AAA_EXT_VER' ) && class_exists( 'Easy_Digital_Downloads' ) ? true : false,
				...
				'learndash'             => array(
					'title'       => 'LearnDash',
					'isActive'    => defined( 'AAA_EXT_VER' ) && class_exists( 'SFWD_LMS' ) ? true : false,
					'description' => __( 'Supercharge your LearnDash website with amazing design features.', 'astra' ),
					'class'       => 'ast-addon',
					'title_url'   => astra_get_pro_url( '/docs/learndash-integration-in-aaa-chinese-pro/', 'free-theme', 'dashboard', 'documentation' ),
					'links'       => array(
						array(
							'link_class'   => 'ast-learn-more',
							'link_url'     => astra_get_pro_url( '/docs/learndash-integration-in-aaa-chinese-pro/', 'free-theme', 'dashboard', 'documentation' ),
							'link_text'    => __( 'Documentation', 'astra' ),
							'target_blank' => true,
						),
					),
				...
				'lifterlms'             => array(
					'title'     => 'LifterLMS',
					'class'     => 'ast-addon',
					'isActive'  => defined( 'AAA_EXT_VER' ) && class_exists( 'LifterLMS' ) ? true : false,
	 ...
	 * //             'icon_path' => "spectra", // If internal_icon false then - example custom SVG URL: AAA_THEME_URI . 'inc/assets/images/astra.svg'.
	 * //         ),
	 * //     ),
	 *
	 * @since 4.0.0
	...
	public function settings_app_scripts( $localize ) {
		...
		$build_path        = AAA_THEME_ADMIN_DIR . 'assets/build/';
		$build_url         = AAA_THEME_ADMIN_URL . 'assets/build/';
	...
	public function astra_admin_footer_link() {
		$theme_name = astra_get_theme_name();
		/** @psalm-suppress TypeDoesNotContainType */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		$rating_url = AAA_THEME_ORG_VERSION ? 'https://wordpress.org/support/theme/astra/reviews/?rate=5#new-post' : 'https://woo.com/products/astra/#reviews';
		/** @psalm-suppress TypeDoesNotContainType */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		if ( astra_is_white_labelled() ) {
			$footer_text = '<span id="footer-thankyou">' . __( 'Thank you for using', 'astra' ) . '<span class="focus:text-aaa-chinese-hover active:text-aaa-chinese-hover hover:text-aaa-chinese-hover"> ' . esc_html( $theme_name ) . '.</span></span>';
		} else {
			$footer_text = sprintf(
