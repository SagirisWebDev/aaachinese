 ...
 * @since 1.0.0
 */
class AAA_Chinese_Icons {
	/**
	 * Constructor function that initializes required actions and hooks
	 */
	public function __construct() {
		if ( self::is_svg_icons() ) {
			add_filter( 'aaa_chinese_enable_default_fonts', '__return_false' );
		}
	}
	...
	public static function is_svg_icons() {
		...
		return apply_filters( 'aaa_chinese_is_svg_icons', isset( $aaa_chinese_settings['can-update-aaa-chinese-icons-svg'] ) ? $aaa_chinese_settings['can-update-aaa-chinese-icons-svg'] : true );
	}
	...
	public static function get_icons( $icon, $is_echo = false, $replace = false, $menu_location = 'main' ) {
		...
		if ( true === self::is_svg_icons() ) {
			...
			if ( 'menu-bars' === $icon ) {
				$menu_icon = apply_filters( 'aaa_chinese_' . $menu_location . '_menu_toggle_icon', 'menu-toggle-icon' );
				$output    = '<span class="' . esc_attr( $menu_icon ) . '"></span>';
			}
		}

		$output = apply_filters( 'aaa_chinese_svg_icon_element', $output, $icon );
		...
		$output = apply_filters(
			'aaa_chinese_svg_icon', // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
			sprintf(
