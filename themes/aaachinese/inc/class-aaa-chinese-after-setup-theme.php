 ...
 *
 * AAA Chinese is a very powerful theme and virtually anything can be customized
 * via a child theme.
 *
 * @package     AAA Chinese
 *
...
/**
 * AAA_Chinese_After_Setup_Theme initial setup
 *
 * @since 1.0.0
 */
if ( ! class_exists( 'AAA_Chinese_After_Setup_Theme' ) ) {

	/**
	 * AAA_Chinese_After_Setup_Theme initial setup
	 */
	class AAA_Chinese_After_Setup_Theme {
		/**
		 * Instance
		 *
		 * @var $instance
		...
		public function setup_theme() {
			...
			do_action( 'aaa_chinese_class_loaded' );
			...
			if ( class_exists( 'AAA_Chinese_Starter_Content', false ) ) {
				$aaa_chinese_starter_content = new AAA_Chinese_Starter_Content();
				add_theme_support( 'starter-content', $aaa_chinese_starter_content->get() );
			}
			...
			$dir_name    = 'minified';
			$file_prefix = '.min';
			if ( apply_filters( 'aaa_chinese_theme_editor_style', true ) ) {
				add_editor_style( 'assets/css/' . $dir_name . '/editor-style' . $file_prefix . '.css' );
			}

			if ( apply_filters( 'aaa_chinese_fullwidth_oembed', true ) ) {
			...
			if ( true === apply_filters( 'aaa_chinese_rank_math_theme_support', true ) ) {
				add_theme_support( 'rank-math-breadcrumbs' );
			}

			// Native AMP Support.
			if ( true === apply_filters( 'aaa_chinese_amp_support', true ) ) {
				add_theme_support(
					'amp',
					apply_filters(
						'aaa_chinese_amp_theme_features',
						array(
							'paired' => true,
						)
					)
			...
			// Let WooCommerce know, AAA Chinese is not compatible with New Product Editor.
			add_filter( 'option_woocommerce_feature_product_block_editor_enabled', '__return_false' );

			add_filter( 'woocommerce_create_pages', array( $this, 'aaa_chinese_enforce_woo_shortcode_pages' ), 99 );
		...
		public function modify_theme_palette_names( $theme_json ) {
			...
			if ( ! empty( $json_data['settings']['color']['palette']['theme'] ) ) {
				...
				$color_labels = AAA_Chinese_Global_Palette::get_palette_labels(); // Use the reusable function for labels.
				$name_pair    = array();
		...
		public function init() {
			 ...
			 */
			load_theme_textdomain( 'aaachinese', AAA_THEME_DIR . '/languages' );
		}
		...
		public function setup_content_width() {
			...
			if ( ! isset( $content_width ) ) {
				...
				if ( is_home() || is_post_type_archive( 'post' ) ) {
					$blog_width = aaa_chinese_get_option( 'blog-width' );

					if ( 'custom' === $blog_width ) {
						$content_width = apply_filters( 'aaa_chinese_content_width', aaa_chinese_get_option( 'blog-max-width', 1200 ) );
					} else {
						$content_width = apply_filters( 'aaa_chinese_content_width', aaa_chinese_get_option( 'site-content-width', 1200 ) );
					}
				} elseif ( is_single() ) {

					if ( 'post' === get_post_type() ) {
						$single_post_max = aaa_chinese_get_option( 'blog-single-width' );

						if ( 'custom' === $single_post_max ) {
							$content_width = apply_filters( 'aaa_chinese_content_width', aaa_chinese_get_option( 'blog-single-max-width', 1200 ) );
						} else {
							$content_width = apply_filters( 'aaa_chinese_content_width', aaa_chinese_get_option( 'site-content-width', 1200 ) );
						}
					}
		...
		public function responsive_oembed_wrapper( $html, $url, $attr, $core_yt_block = false ) {
			...
			$add_aaa_chinese_oembed_wrapper = apply_filters( 'aaa_chinese_responsive_oembed_wrapper_enable', true );
			$aaa_chinese_embed_wrapper_class  = apply_filters( 'aaa_chinese_embed_wrapper_class', '' );

			$allowed_providers = apply_filters(
				'aaa_chinese_allowed_fullwidth_oembed_providers',
				array(
					'vimeo.com',
					'youtube.com',
					'youtu.be',
			...
			if ( aaa_chinese_strposa( $url, $allowed_providers ) && $add_aaa_chinese_oembed_wrapper ) {
				if ( $core_yt_block ) {
					$embed_html = wp_oembed_get( $url );
					$html       = false !== $embed_html ? '<div class="wp-block-embed__wrapper"> <div class="aaachinese-oembed-container ' . esc_attr( $aaa_chinese_embed_wrapper_class ) . '" style="height: 100%;">' . $embed_html . '</div> </div>' : '';
				} else {
					$html = '' !== $html ? '<div class="aaachinese-oembed-container ' . esc_attr( $aaa_chinese_embed_wrapper_class ) . '" style="height: 100%;">' . $html . '</div>' : '';
				}
		 ...
		 * 2. Due to which most of AAA Chinese extended features are not working on cart & checkout pages.
		 *
		 * This is temporary workaround, once AAA Chinese ready with WooCommerce 8.3 version, this will be removed.
		 *
		 * @since 4.5.1
		 ...
		 * @return array
		 */
		public function aaa_chinese_enforce_woo_shortcode_pages( $pages_data ) {
 ...
 */
AAA_Chinese_After_Setup_Theme::get_instance();
