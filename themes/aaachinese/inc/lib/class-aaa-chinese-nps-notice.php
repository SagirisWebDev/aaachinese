...
if ( ! class_exists( 'AAA_Chinese_Nps_Notice' ) ) {

	/**
	 * Admin
	 */
	class AAA_Chinese_Nps_Notice {
		/**
		 * Instance
		 *
		 * @since 1.0.0
		 * @var (Object) AAA_Chinese_Nps_Notice
		 */
		...
		private function __construct() {
			...
			if ( ! current_user_can( 'manage_options' ) || apply_filters( 'aaa_chinese_nps_survey_disable', false ) ) {
				return;
			}
			...
			if (
				defined( 'AAA_EXT_VER' ) &&
				is_callable( 'AAA_Chinese_Ext_White_Label_Markup::get_whitelabel_string' ) &&
				'astra' !== strtolower( AAA_Chinese_Ext_White_Label_Markup::get_whitelabel_string( 'astra', 'name', 'astra' ) )
			) {
				return;
			}
			...
			add_action( 'admin_footer', array( $this, 'render_aaa_chinese_nps_survey' ), 999 );
		}
		 ...
		 * @return void
		 */
		public function render_aaa_chinese_nps_survey() {
			...
			$allowed_screens = [
				'toplevel_page_astra',
				'aaa_chinese_page_theme-builder-free',
				'aaa_chinese_page_theme-builder',
			];
			...
			Nps_Survey::show_nps_notice(
				'nps-survey-astra',
				array(
					'show_if' => defined( 'AAA_THEME_VERSION' ),
					'dismiss_timespan' => 2 * WEEK_IN_SECONDS,
					'display_after' => get_option('aaa_chinese_nps_show') ? 0 : 2 * WEEK_IN_SECONDS,
					'plugin_slug' => 'astra',
					'show_on_screens' => $allowed_screens,
					'message' => array(
						// Step 1 i.e rating input.
						'logo' => esc_url( AAA_THEME_URI . 'inc/assets/images/astra-logo.svg'),
						'plugin_name' => __( 'AAA Chinese', 'astra' ),
						'nps_rating_title' => __( 'Quick Question', 'astra' ),
						'nps_rating_message' => __( 'How would you rate AAA Chinese? Love it, hate it, or somewhere in between? Your honest answer helps us understand how were doing.', 'astra' ),
						...
						'feedback_content' => __( 'Thanks for using AAA Chinese! Got feedback or suggestions to make it even better? We’d love to hear from you.', 'astra' ),
	 ...
	 */
	AAA_Chinese_Nps_Notice::get_instance();
