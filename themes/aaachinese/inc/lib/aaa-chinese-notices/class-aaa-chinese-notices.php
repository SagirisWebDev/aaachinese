...
if ( ! class_exists( 'AAA_Chinese_Notices' ) ) :
	...
	class AAA_Chinese_Notices {
		...
		public static function add_notice( $args = array() ) {
			...
			$notices   = get_option( 'allowed_aaa_chinese_notices', array() );
			if ( ! in_array( $notice_id, $notices, true ) ) {
				$notices[] = $notice_id; // Add notice id to the array.
				update_option( 'allowed_aaa_chinese_notices', $notices ); // Update the option.
			}
		}
		...
		public function dismiss_notice() {
			...
			if ( ! apply_filters( 'aaa_chinese_notices_user_cap_check', current_user_can( $capability ) ) ) {
				return;
			}

			$allowed_notices = get_option( 'allowed_aaa_chinese_notices', array() ); // Get allowed notices.
		...
		public function show_notices() {
			$defaults = array(
				...
				'display-with-other-notices' => true,    // Should the notice be displayed if other notices  are being displayed from AAA_Chinese_Notices.
				'is_dismissible'             => true,
		...
		public static function markup( $notice = array() ) {
			...
			do_action( 'aaa_chinese_notice_before_markup' );

			do_action( "aaa_chinese_notice_before_markup_{$notice['id']}" );
			...
			<div id="<?php echo esc_attr( $notice['id'] ); ?>" class="<?php echo 'astra-notice-wrapper ' . esc_attr( $notice['classes'] ); ?>" data-repeat-notice-after="<?php echo esc_attr( $notice['repeat-notice-after'] ); ?>">
				<div class="astra-notice-container">
					<?php do_action( "aaa_chinese_notice_inside_markup_{$notice['id']}" ); ?>
					<?php echo wp_kses_post( $notice['message'] ); ?>
				</div>
			</div>
			<?php

			do_action( "aaa_chinese_notice_after_markup_{$notice['id']}" );

			do_action( 'aaa_chinese_notice_after_markup' );
		}
	 ...
	 */
	AAA_Chinese_Notices::get_instance();
