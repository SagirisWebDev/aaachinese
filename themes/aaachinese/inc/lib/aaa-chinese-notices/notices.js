...
( function( $ ) {
	...
	AAA ChineseNotices = {
		...
		/**
		 * Binds events for the AAA Chinese Portfolio.
		 *
		 * @since 1.0.0
		 * @access private
		 * @method _bind
		...
		_bind: function()
		{
			$( document ).on('click', '.astra-notice-close', AAA ChineseNotices._dismissNoticeNew );
			$( document ).on('click', '.astra-notice .notice-dismiss', AAA ChineseNotices._dismissNotice );
		},
		...
		_dismissNotice: function( event ) {
			...
			AAA ChineseNotices._ajax( notice_id, repeat_notice_after );
		},
