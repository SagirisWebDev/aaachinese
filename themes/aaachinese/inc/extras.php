 ...
 */
if ( ! function_exists( 'aaa_chinese_body_font_family' ) ) {
	 ...
	 * @return string
	 */
	function aaa_chinese_body_font_family() {
		...
		return apply_filters( 'aaa_chinese_body_font_family', $font_family );
	}
}
 ...
 */
if ( ! function_exists( 'aaa_chinese_header_breakpoint_style' ) ) {
	 ...
	 * @since 1.0.0
	 */
	function aaa_chinese_header_breakpoint_style( $dynamic_css, $dynamic_css_filtered = '' ) {

		$aaa_chinese_header_width = aaa_chinese_get_option( 'header-main-layout-width' );

		if ( 'content' === $aaa_chinese_header_width ) {
			return $dynamic_css;
		}

		// Header Break Point.
		$header_break_point = aaa_chinese_header_break_point();
		...
		$dynamic_css .= aaa_chinese_parse_css( $genral_global_responsive );
		$dynamic_css .= aaa_chinese_parse_css( $padding_below_breakpoint, '', $header_break_point );

		// trim white space for faster page loading.
		$dynamic_css .= AAA_Chinese_Enqueue_Scripts::trim_css( $dynamic_css );

		return $dynamic_css;
	}
}

add_filter( 'aaa_chinese_dynamic_theme_css', 'aaa_chinese_header_breakpoint_style' );

/**
 * Function to filter comment form arguments
 */
if ( ! function_exists( 'aaa_chinese_404_page_layout' ) ) {
	 ...
	 * @return array
	 */
	function aaa_chinese_404_page_layout( $layout ) {

		if ( is_404() ) {
			$layout = 'no-sidebar';
		}

		return apply_filters( 'aaa_chinese_404_page_layout', $layout );
	}
}

add_filter( 'aaa_chinese_page_layout', 'aaa_chinese_404_page_layout', 10, 1 );

/**
 * Function to check logo component present in which Header section.
 */
if ( ! function_exists( 'aaa_chinese_is_logo_in_section' ) ) {
	...
	function aaa_chinese_is_logo_in_section( $section ) {
 ...
 */
if ( ! function_exists( 'aaa_chinese_get_content_layout' ) ) {
	 ...
	 */
	function aaa_chinese_get_content_layout() {

		if ( is_singular() ) {

			// Old meta layout migration to new layout.
			$old_meta_content_layout = aaa_chinese_get_option_meta( 'site-content-layout', '', true );

			if ( $old_meta_content_layout ) {
				$content_layout = aaa_chinese_toggle_layout( 'ast-site-content-layout', 'meta', false, $old_meta_content_layout );
			} else {
				$content_layout = aaa_chinese_get_option_meta( 'ast-site-content-layout', '', true );

				// If post meta value is present, apply new layout option.
				if ( $content_layout ) {
					$content_layout = aaa_chinese_toggle_layout( 'ast-site-content-layout', 'meta', false );
				}
			}
			...
			if ( empty( $content_layout ) ) {
				...
				$content_layout = aaa_chinese_toggle_layout( 'single-' . $post_type . '-ast-content-layout', 'single', false );
			...
			$content_layout = aaa_chinese_toggle_layout( 'archive-' . $post_type . '-ast-content-layout', 'archive', false );

			if ( is_search() ) {
				$search_content_layout = aaa_chinese_get_option( 'ast-search-content-layout', 'default' );
				$content_layout_key    = 'default' !== $search_content_layout ? 'ast-search-content-layout' : 'archive-post-ast-content-layout';
				$content_layout        = aaa_chinese_toggle_layout( $content_layout_key, 'archive', false );
			}
		...
		return apply_filters( 'aaa_chinese_get_content_layout', $content_layout );
	}
}
 ...
 * @return mixed content layout.
 */
function aaa_chinese_toggle_layout( $new_content_option, $level, $post_id = false, $old_meta = false ) {

	// Dynamic layout option for meta case.
	$dynamic_layout_option = 'meta' === $level ? aaa_chinese_get_option_meta( $new_content_option, '', true ) : aaa_chinese_get_option( $new_content_option, 'post' === strval( get_post_type() ) ? 'narrow-width-container' : 'default' );
	$current_layout        = '';
	...
	if ( 'meta' === $level && $migrated_user ) {
		...
		if ( $old_meta && 'set' !== $meta_key ) {
			$dynamic_layout_option = aaa_chinese_migrate_meta_layout( $old_meta );
		}
	}
	...
	switch ( $dynamic_layout_option ) {
		...
		case 'narrow-width-container':
			...
			if ( aaa_chinese_with_third_party() && ( ! $migrated_user ) ) {
				$current_layout = 'plain-container';
			}
			break;
 ...
 */
function aaa_chinese_migrate_meta_layout( $meta_layout ) {
	$new_layout = '';
	switch ( $meta_layout ) {
 ...
 */
if ( ! function_exists( 'aaa_chinese_check_is_ie' ) ) {
	 ...
	 */
	function aaa_chinese_check_is_ie() {
		...
		return apply_filters( 'aaa_chinese_check_is_ie', $is_ie );
	}
}
 ...
 * Replace header logo.
 */
if ( ! function_exists( 'aaa_chinese_replace_header_logo' ) ) {
	 ...
	 */
	function aaa_chinese_replace_header_logo( $image, $attachment_id, $size, $icon ) {
		...
		return apply_filters( 'aaa_chinese_replace_header_logo', $image );
	}
}

if ( ! function_exists( 'aaa_chinese_strposa' ) ) {
	 ...
	 */
	function aaa_chinese_strposa( $haystack, $needles, $offset = 0 ) {
...
if ( ! function_exists( 'aaa_chinese_get_prop' ) ) {
	 ...
	 */
	function aaa_chinese_get_prop( $array, $prop, $default = null ) {
 ...
 */
function aaa_chinese_attr( $context, $attributes = array(), $args = array() ) {
	return AAA_Chinese_Attr::get_instance()->aaa_chinese_attr( $context, $attributes, $args );
}
 ...
 */
function aaa_chinese_get_theme_author_details() {
	return apply_filters(
		'aaa_chinese_theme_author',
		array(
			'theme_name'       => esc_html__( 'AAA Chinese WordPress Theme', 'astra' ),
 ...
 */
function aaa_chinese_dropdown_icon_to_menu_link( $title, $item, $args, $depth ) {
	$role = 'button';
	$icon = '';

	/**
	 * These menus are not overriden by the 'AAA_Chinese_Custom_Nav_Walker' class present in Addon - Nav Menu module.
	 *
	 * Hence skipping these menus from getting overriden by blank SVG Icons and adding the icons from theme.
	 *
	 * @since 3.3.0
	 */
	$aaa_chinese_menu_locations = array(
	...
	if ( defined( 'AAA_EXT_VER' ) ) {
		// Check whether AAA Chinese Pro is active + Nav menu addon is deactivate + menu registered by AAA Chinese only.
		if ( ! AAA_Chinese_Ext_Extension::is_active( 'nav-menu' ) && isset( $args->menu_id ) && in_array( $args->menu_id, $aaa_chinese_menu_locations ) ) {
			$load_svg_menu_icons = true;
		}
	} else {
		// Check menu registered by AAA Chinese only.
		if ( isset( $args->menu_id ) && in_array( $args->menu_id, $aaa_chinese_menu_locations ) ) {
			$load_svg_menu_icons = true;
		}
	}

	if ( $load_svg_menu_icons || ( defined( 'AAA_EXT_VER' ) && ! AAA_Chinese_Ext_Extension::is_active( 'nav-menu' ) ) ) {
		// Assign icons to only those menu which are registered by AAA Chinese.
		$icon = wp_kses(
			/**
			 ...
			 * @since 4.12.1
			 */
			apply_filters( 'aaa_chinese_mobile_nav_submenu_items_arrow_icon', AAA_Chinese_Icons::get_icons( 'arrow' ) ),
			AAA_Chinese_Icons::allowed_svg_args()
		);
	}

	// Dropdown svg arrow for submenu for non AAA Chinese registered menu's
	$is_nav_menu_extension_inactive = defined( 'AAA_EXT_VER' ) && ! AAA_Chinese_Ext_Extension::is_active( 'nav-menu' );
	...
	if ( $is_nav_menu_extension_inactive || $is_special_menu ) {
		$icon = AAA_Chinese_Icons::get_icons( 'arrow' );
	}

	$aaa_chinese_arrow_icon = '';
	// Render arrow icon for special menu appearance or on pro deactivation or nav menu extension deactivation.
	if ( $is_special_menu || $is_nav_menu_extension_inactive || ! defined( 'AAA_EXT_VER' ) ) {
		$custom_tabindex  = true === AAA_Chinese_Builder_Helper::$is_header_footer_builder_active ? 'tabindex="0"' : '';
		$aaa_chinese_arrow_icon = '<span role="' . esc_attr( $role ) . '" class="dropdown-menu-toggle ast-header-navigation-arrow" ' . $custom_tabindex . ' aria-expanded="false" aria-label="' . esc_attr__( 'Menu Toggle', 'astra' ) . '" aria-haspopup="true">' . $icon . '</span>';
	}

	foreach ( $item->classes as $value ) {
		if ( 'menu-item-has-children' === $value ) {
			$title .= $aaa_chinese_arrow_icon;
		}
	}
	if ( 0 < $depth ) {
		$title = $icon . $title;
...
if ( AAA_Chinese_Icons::is_svg_icons() ) {
	add_filter( 'nav_menu_item_title', 'aaa_chinese_dropdown_icon_to_menu_link', 10, 4 );
}
 ...
 */
function aaa_chinese_existing_header_footer_configs() {

	return apply_filters( 'aaa_chinese_existing_header_footer_configs', true );
}
 ...
 * @return mixed
 */
function aaa_chinese_calculate_spacing( $value, $operation = '', $from = '', $from_unit = '' ) {
 ...
 * @return mixed
 */
function aaa_chinese_markup_open( $context, $args = array() ) {
	$defaults = array(
		'open'    => '',
		'attrs'   => array(),
		'echo'    => true,
	...
	if ( $context ) {
		$args     = apply_filters( "aaa_chinese_markup_{$context}_open", $args );
		$open_tag = $args['open'] ? sprintf( $args['open'], aaa_chinese_attr( $context, $args['attrs'] ) ) : '';
 ...
 * @return mixed
 */
function aaa_chinese_markup_close( $context, $args = array() ) {
	$defaults = array(
		'close' => '',
		'attrs' => array(),
		'echo'  => true,
	...
	$args = wp_parse_args( $args, $defaults );
	if ( $context ) {
		$args      = apply_filters( "aaa_chinese_markup_{$context}_close", $args );
...
/**
 * Provision to update display rules for visibility of Related Posts section in AAA Chinese.
 *
 * @since 3.4.0
 * @return bool
 */
function aaa_chinese_target_rules_for_related_posts() {

	$allow_related_posts = false;
	$supported_post_type = apply_filters( 'aaa_chinese_related_posts_supported_post_types', 'post' );

	if ( aaa_chinese_get_option( 'enable-related-posts' ) && is_singular( $supported_post_type ) ) {
		$allow_related_posts = true;
	}

	return apply_filters( 'aaa_chinese_showcase_related_posts', $allow_related_posts );
}
 ...
 * @return bool
 */
function aaa_chinese_is_elemetor_active() {
	return class_exists( '\Elementor\Plugin' );
}

/**
 * Check the AAA Chinese addon version.
 * For  major update and frequently we used version_compare, added a function for this for easy maintenance.
 *
 * @param string $version AAA Chinese addon version.
 * @param string $compare Compare symbols.
 * @since  3.9.2
 */
function aaa_chinese_addon_check_version( $version, $compare ) {
	return defined( 'AAA_EXT_VER' ) && version_compare( AAA_EXT_VER, $version, $compare );
}
 ...
 */
function aaa_chinese_get_webfont_url( $url, $format = 'woff2' ) {

	// Check if already Google font URL present or not. Basically avoiding 'AAA_Chinese_WebFont_Loader' class rendering.
	/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
	$aaa_chinese_font_url = aaa_chinese_get_option( 'aaa_chinese_font_url', false );
	if ( $aaa_chinese_font_url ) {
		return json_decode( $aaa_chinese_font_url );
	}

	// Now create font URL if its not present.
	$font = aaa_chinese_webfont_loader_instance( $url );
 ...
 * @param string $format The font-format. If you need to support IE, change this to "woff".
 */
function aaa_chinese_load_preload_local_fonts( $url, $format = 'woff2' ) {

	// Check if cached font files data preset present or not. Basically avoiding 'AAA_Chinese_WebFont_Loader' class rendering.
	$aaa_chinese_local_font_files = get_site_option( 'aaa_chinese_local_font_files', false );

	if ( is_array( $aaa_chinese_local_font_files ) && ! empty( $aaa_chinese_local_font_files ) ) {
		$font_format = apply_filters( 'aaa_chinese_local_google_fonts_format', $format );
		foreach ( $aaa_chinese_local_font_files as $local_font ) {
			if ( $local_font ) {
 ...
 * @since 3.6.0
 */
function aaa_chinese_get_transparent_header_default_value() {
	$aaa_chinese_settings = aaa_chinese_get_options();
	return apply_filters( 'aaa_chinese_transparent_header_default_border', isset( $aaa_chinese_settings['transparent-header-default-border'] ) ? $aaa_chinese_settings['transparent-header-default-border'] : true );
}
 ...
 * @since 3.7.0
 */
function aaa_chinese_has_gcp_typo_preset_compatibility() {
	if ( defined( 'AAA_EXT_VER' ) && version_compare( AAA_EXT_VER, '3.6.0', '<' ) ) {
		return false;
	}
	return true;
}
 ...
 * @return string
 */
function aaa_chinese_button_default_padding_updated() {
	$aaa_chinese_settings = aaa_chinese_get_options();
	return apply_filters( 'aaa_chinese_update_button_padding_defaults', isset( $aaa_chinese_settings['btn-default-padding-updated'] ) ? $aaa_chinese_settings['btn-default-padding-updated'] : true );
}
 ...
 * @return string
 */
function aaa_chinese_scndry_btn_default_padding() {
	$aaa_chinese_settings = aaa_chinese_get_options();
	return apply_filters( 'aaa_chinese_update_secondary_button_padding_defaults', isset( $aaa_chinese_settings['scndry-btn-default-padding'] ) ? $aaa_chinese_settings['scndry-btn-default-padding'] : true );
}
 ...
 * @return bool
 */
function aaa_chinese_has_widgets_block_editor() {
 ...
 * @return bool
 */
function aaa_chinese_can_remove_elementor_toc_margin_space() {
	$aaa_chinese_settings = aaa_chinese_get_options();
	return apply_filters( 'aaa_chinese_remove_elementor_toc_margin', isset( $aaa_chinese_settings['remove-elementor-toc-margin-css'] ) ? false : true );
}
 ...
 * @return bool
 */
function aaa_chinese_can_add_styling_for_hr() {
	$aaa_chinese_settings = aaa_chinese_get_options();
	return apply_filters( 'aaa_chinese_highlight_elementor_hr_tag', isset( $aaa_chinese_settings['add-styling-for-hr'] ) ? false : true );
}
/**
 ...
 */
function aaa_chinese_has_global_color_format_support() {
	$aaa_chinese_settings = aaa_chinese_get_options();
	return apply_filters( 'aaa_chinese_apply_global_color_format_support', isset( $aaa_chinese_settings['support-global-color-format'] ) ? false : true );
}
 ...
 * @since 3.7.0
 */
function aaa_chinese_get_palette_colors() {
	$color_palettes = get_option( 'astra-color-palettes', false );

	if ( ! $color_palettes ) {
		$color_palettes = \AAA_Chinese_Global_Palette::get_default_color_palette();
	}

	if ( ! isset( $color_palettes['palettes']['palette_4'] ) ) {
		$default_palette                         = \AAA_Chinese_Global_Palette::get_default_color_palette();
		$color_palettes['palettes']['palette_4'] = $default_palette['palettes']['palette_4'] ?? '';
	}

	$presets = aaa_chinese_get_palette_presets();

	$color_palettes['presets'] = $presets;

	// Return the color palettes and presets together as a single array
	return apply_filters( 'aaa_chinese_global_color_palette', $color_palettes );
}
 ...
 * @since 4.10.0
 */
function aaa_chinese_get_palette_names() {
	 ...
	 */
	return apply_filters( 'aaa_chinese_get_palette_names', $palette_names );
}
 ...
 * @since 3.7.0
 */
function aaa_chinese_get_typography_presets() {
	return get_option( 'astra-typography-presets', '' );
}

/**
 * Clear AAA Chinese + AAA Chinese Pro assets cache.
 *
 * @since 3.6.9
 * @return void
 */
function aaa_chinese_clear_theme_addon_asset_cache() {
	aaa_chinese_clear_all_assets_cache();
}

add_action( 'aaa_chinese_theme_update_after', 'aaa_chinese_clear_theme_addon_asset_cache', 10 );
 ...
 * @return bool
 */
function aaa_chinese_maybe_disable_global_color_in_elementor() {
	return apply_filters( 'aaa_chinese_disable_global_colors_in_elementor', false );
}
 ...
 * @return bool
 */
function aaa_chinese_check_elementor_pro_3_5_version() {
 ...
 * @return bool
 */
function aaa_chinese_apply_content_background_fullwidth_layouts() {
	$aaa_chinese_site_layout              = aaa_chinese_get_option( 'site-layout' );
	$aaa_chinese_apply_content_background = aaa_chinese_get_option( 'apply-content-background-fullwidth-layouts', true );
	return $aaa_chinese_apply_content_background && 'ast-box-layout' !== $aaa_chinese_site_layout && 'ast-padded-layout' !== $aaa_chinese_site_layout;
}
 ...
 * @since 3.5.0
 */
function aaa_chinese_search_static_css() {
	$search_css = '
	...
	return AAA_Chinese_Enqueue_Scripts::trim_css( $search_css );
}

/**
 * Showcase "Upgrade to Pro" notices for AAA Chinese & here is the filter work as central control to enable/disable those notices from customizer, meta settings, admin area, pro post types pages.
 *
 * @since 3.9.4
 * @return bool
 */
function aaa_chinese_showcase_upgrade_notices() {
	return ! defined( 'AAA_EXT_VER' ) && aaa_chinese_get_option( 'ast-disable-upgrade-notices', true ) ? true : false;
}
 ...
 * @since 4.0.0
 */
function aaa_chinese_get_font_extras( $config, $setting, $unit = false ) {
	$css = isset( $config[ $setting ] ) ? $config[ $setting ] : '';

	if ( $unit && $css ) {
		$unit_val = isset( $config[ $unit ] ) ? $config[ $unit ] : '';
		$unit_val = 'line-height-unit' === $unit ? apply_filters( 'aaa_chinese_font_line_height_unit', $unit_val ) : $unit_val;
		$css     .= $unit_val;
	}
 ...
 * @since 4.0.0
 */
function aaa_chinese_get_font_array_css( $font_family, $font_weight, $font_size, $font_extras, $color = '' ) {
	$font_extras_ast_option = aaa_chinese_get_option(
		$font_extras,
		array(
			'line-height'         => '',
			'line-height-unit'    => 'em',
	...
	return array(
		...
		'font-family'     => aaa_chinese_get_css_value( $font_family, 'font' ),
		'font-weight'     => aaa_chinese_get_css_value( $font_weight, 'font' ),
		'font-size'       => ! empty( $font_size ) ? aaa_chinese_responsive_font( $font_size, 'desktop' ) : '',
		'line-height'     => aaa_chinese_get_font_extras( $font_extras_ast_option, 'line-height', 'line-height-unit' ),
		'text-transform'  => aaa_chinese_get_font_extras( $font_extras_ast_option, 'text-transform' ),
		'letter-spacing'  => aaa_chinese_get_font_extras( $font_extras_ast_option, 'letter-spacing', 'letter-spacing-unit' ),
		'text-decoration' => aaa_chinese_get_font_extras( $font_extras_ast_option, 'text-decoration' ),
	);
}
 ...
 * @return array
 */
function aaa_chinese_get_site_image_sizes( $add_custom = false ) {
	$image_sizes = array(
	...
	if ( $add_custom && defined( 'AAA_EXT_VER' ) && AAA_Chinese_Ext_Extension::is_active( 'blog-pro' ) ) {
		/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
 ...
 * @return string
 */
function aaa_chinese_get_dynamic_image_aspect_ratio( $aspect_ratio_type, $predefined_scale, $custom_scale_width, $custom_scale_height ) {
 ...
 * @return string
 */
function aaa_chinese_get_current_language_slug() {
	$lang = '';
	if ( function_exists( 'pll_current_language' ) ) {
		$lang = pll_current_language();
	}
	return apply_filters( 'aaa_chinese_addon_site_current_language', $lang );
}
 ...
 * @return array
 */
function aaa_chinese_get_queried_post_types() {
	$queried_post_types = array_keys(
		get_post_types(
			apply_filters(
				'aaa_chinese_dynamic_get_post_types_query_args',
				array(
					'public'   => true,
					'_builtin' => false,
				)
 ...
 * @since 4.5.0
 */
function aaa_chinese_get_palette_presets() {
	$color_palette_reorganize = AAA_Chinese_Dynamic_CSS::aaa_chinese_4_8_0_compatibility();
	return array(
		'Oak'    => array(
			'#0067FF',
			'#005EE9',
...
/**
 * Get AAA Chinese blog layout design.
 * Search / Blog.
 *
 * @return string $blog_layout.
 * @since 4.6.0
 */
function aaa_chinese_get_blog_layout() {
	return is_search() && '' !== aaa_chinese_get_option( 'ast-search-results-style' ) ? aaa_chinese_get_option( 'ast-search-results-style' ) : aaa_chinese_get_option( 'blog-layout' );
}

/**
 * Get AAA Chinese blog posts per page count.
 * Search / Blog.
 *
 * @return int $blog_layout.
 * @since 4.6.0
 */
function aaa_chinese_get_blog_posts_per_page() {
	$posts_per_page = is_search() && aaa_chinese_get_option( 'ast-search-results-per-page' )
		? aaa_chinese_get_option( 'ast-search-results-per-page' )
		: aaa_chinese_get_option( 'blog-post-per-page' );

	return is_numeric( $posts_per_page ) && $posts_per_page > 0 ? intval( $posts_per_page ) : 10;
}

/**
 * Get the remote WP-AAA Chinese docs data.
 *
 * @since 4.6.0
 */
function aaa_chinese_remote_docs_data() {
	/**
	 ...
	 * @since 4.12.2
	 */
	if ( apply_filters( 'aaa_chinese_disable_docs', true ) ) {
		return false;
	}

	$aaa_chinese_docs_instance = aaa_chinese_docs_loader_instance( AAA_WEBSITE_BASE_URL . '/wp-json/powerful-docs/v1/get-docs', 'astra-docs' );
	return json_decode( $aaa_chinese_docs_instance->get_remote_data() );
}
 ...
 * @since 4.4.0
 */
function aaa_chinese_customizer_live_search_posttypes() {
	$supported_post_types = array();
	if ( is_customize_preview() ) {
		$supported_post_types = aaa_chinese_get_queried_post_types();
	}
	return apply_filters( 'aaa_chinese_live_search_posttypes', $supported_post_types );
}
 ...
 * @return array
 */
function aaa_chinese_customizer_search_post_types_choices() {
	$all_post_types    = aaa_chinese_customizer_live_search_posttypes();
 ...
 * @return bool
 */
function aaa_chinese_enable_edd_featured_image_defaults() {
	$aaa_chinese_settings = aaa_chinese_get_options();
	return apply_filters( 'aaa_chinese_enable_edd_featured_image_defaults', isset( $aaa_chinese_settings['can-update-edd-featured-image-default'] ) ? false : true );
}
