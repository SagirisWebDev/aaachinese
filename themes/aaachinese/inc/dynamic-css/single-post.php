<?php
/**
 * Single Post UI Improvement - Dynamic CSS
 *
 * @package astra-builder
 * @since 4.6.0
 */

if ( ! defined( 'ABSPATH' ) ) {
	exit; // Exit if accessed directly.
}

if ( apply_filters( 'aaa_chinese_improvise_single_post_design', AAA_Chinese_Dynamic_CSS::aaa_chinese_4_6_0_compatibility() && is_single() ) ) {
	add_filter( 'aaa_chinese_dynamic_theme_css', 'aaa_chinese_single_post_css', 11 );
}

/**
 * Single Post UI Improvement - Dynamic CSS
 *
 * @param  string $dynamic_css          AAA Chinese Dynamic CSS.
 * @return String Generated dynamic CSS for Pagination.
 *
 * @since 4.6.0
 */
function aaa_chinese_single_post_css( $dynamic_css ) {
	$is_boxed       = aaa_chinese_is_content_style_boxed();
	$content_layout = aaa_chinese_get_content_layout();

	$post_with_unboxed_layout = ( 'plain-container' === $content_layout || 'narrow-container' === $content_layout ) && ! $is_boxed ? true : false;

	$static_css = '
		:root {
			--ast-single-post-border: #e1e8ed;
		}
		.entry-content > * {
			margin-bottom: 1.5em;
		}
		.entry-content :where(h1, h2, h3, h4, h5, h6) {
			margin-top: 1.5em;
			margin-bottom: calc(0.3em + 10px);
		}
		code, kbd, samp {
			background: var(--ast-code-block-background);
			padding: 3px 6px;
		}
		.ast-row.comment-textarea fieldset.comment-form-comment {
			border: none;
			padding: unset;
			margin-bottom: 1.5em;
		}
		.entry-content .wp-block-image,
		.entry-content .wp-block-embed {
			margin-top: 2em;
			margin-bottom: 3em;
		}
	';

	if ( $post_with_unboxed_layout ) {
		$static_css .= '
			:root {
				--ast-single-post-nav-padding: 4em 0 0;
			}
			.ast-single-post .ast-post-format-content {
				max-width: 100%;
			}
			.post-navigation + .comments-area {
				border-top: none;
				padding-top: 5em;
			}
		';
	} else {
		$nav_padding = aaa_chinese_check_current_post_comment_enabled() || 0 < get_comments_number() ? '3em 0 1em' : '3em 0 0';
		$static_css .= '
			:root {
				--ast-single-post-nav-padding: ' . $nav_padding . ';
			}
		';
	}

	$dynamic_css .= AAA_Chinese_Enqueue_Scripts::trim_css( $static_css );

	return $dynamic_css;
}
