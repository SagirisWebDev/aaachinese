...
add_filter( 'aaa_chinese_dynamic_theme_css', 'aaa_chinese_inline_on_mobile_css' );
 ...
 * @return string
 */
function aaa_chinese_inline_on_mobile_css( $dynamic_css ) {

	$inline_on_mobile_enable = false;
	for ( $index = 1; $index <= AAA_Chinese_Builder_Helper::$component_limit; $index++ ) {
		if ( false === aaa_chinese_get_option( 'header-menu' . $index . '-menu-stack-on-mobile' ) ) {
	...
	return $dynamic_css . AAA_Chinese_Enqueue_Scripts::trim_css( $inline_on_mobile_css );
}
