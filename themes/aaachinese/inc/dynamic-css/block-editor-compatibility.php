<?php
/**
 * AAA Chinese WordPress-5.8 compatibility - Dynamic CSS.
 *
 * @package astra
 * @since 3.6.5
 */
...
if ( aaa_chinese_block_based_legacy_setup() ) {
	add_filter( 'aaa_chinese_dynamic_theme_css', 'aaa_chinese_block_editor_compatibility_css' );
} else {
	add_filter( 'aaa_chinese_dynamic_theme_css', 'aaa_chinese_load_modern_block_editor_ui' );
}
 ...
 * @since 3.6.5
 */
function aaa_chinese_get_block_editor_required_css() {
	return '
		.wp-block-file {
			display: flex;
			align-items: center;
...
/**
 * AAA Chinese WordPress compatibility - Dynamic CSS.
 *
 * @param string $dynamic_css Dynamic CSS.
 * @since 3.6.5
 */
function aaa_chinese_block_editor_compatibility_css( $dynamic_css ) {

	if ( AAA_Chinese_Dynamic_CSS::is_block_editor_support_enabled() ) {
		...
		$dynamic_css .= AAA_Chinese_Enqueue_Scripts::trim_css( $compatibility_css );
	}

	/** @psalm-suppress InvalidScalarArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
	if ( aaa_chinese_get_option( 'improve-gb-editor-ui', true ) ) {
		/** @psalm-suppress InvalidScalarArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		$is_site_rtl = is_rtl();

		$editor_improvement_css = aaa_chinese_get_block_editor_required_css();
	...
	$dynamic_css .= AAA_Chinese_Enqueue_Scripts::trim_css( $editor_improvement_css );

	return $dynamic_css;
}

/**
 * AAA Chinese block editor 2.0 Spectra compatibility - Dynamic CSS.
 *
 * @param string $dynamic_css Dynamic CSS.
 ...
 * @since 3.8.0
 */
function aaa_chinese_load_modern_block_editor_ui( $dynamic_css ) {
	$dynamic_css             .= aaa_chinese_get_block_editor_required_css();
	$ltr_left                 = is_rtl() ? 'right' : 'left';
	$ltr_right                = is_rtl() ? 'left' : 'right';
	$aaa_chinese_block_editor_v2_ui = aaa_chinese_get_option( 'wp-blocks-v2-ui', true );
	$ast_container_width      = aaa_chinese_get_option( 'site-content-width', 1200 ) . 'px';
	$blocks_spacings          = AAA_Chinese_WP_Editor_CSS::aaa_chinese_get_block_spacings();

	/** @psalm-suppress InvalidCast */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
	$tablet_breakpoint = (string) aaa_chinese_get_tablet_breakpoint();
	/** @psalm-suppress InvalidCast */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort

	/** @psalm-suppress InvalidCast */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
	$mobile_breakpoint = (string) aaa_chinese_get_mobile_breakpoint();
	/** @psalm-suppress InvalidCast */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
	...
	$ast_content_width = apply_filters( 'aaa_chinese_block_content_width', $aaa_chinese_block_editor_v2_ui ? $ast_container_width : '910px' );
	$ast_wide_width    = apply_filters( 'aaa_chinese_block_wide_width', $aaa_chinese_block_editor_v2_ui ? 'calc(' . esc_attr( $ast_container_width ) . ' + var(--wp--custom--ast-default-block-left-padding) + var(--wp--custom--ast-default-block-right-padding))' : $ast_container_width );
	...
	$spectra_gutenberg_compat_css = AAA_Chinese_Dynamic_CSS::spectra_gutenberg_compat_css();
	$v4_block_editor_compat       = AAA_Chinese_Dynamic_CSS::v4_block_editor_compat();
	...
	$aaa_chinese_wide_particular_selector = $aaa_chinese_block_editor_v2_ui ? '.entry-content[data-ast-blocks-layout] > .alignwide' : '.entry-content[data-ast-blocks-layout] > .alignwide, .entry-content[data-ast-blocks-layout] .wp-block-cover__inner-container, .entry-content[data-ast-blocks-layout] > p';
	$aaa_chinese_full_stretched_selector  = $aaa_chinese_block_editor_v2_ui ? '.ast-plain-container.ast-no-sidebar .entry-content > .alignfull, .ast-page-builder-template .ast-no-sidebar .entry-content > .alignfull' : '.ast-plain-container.ast-no-sidebar .entry-content .alignfull, .ast-page-builder-template .ast-no-sidebar .entry-content .alignfull';
	...
	' . $aaa_chinese_full_stretched_selector . ' {
	...
	' . $aaa_chinese_wide_particular_selector . ' {
	...
	if ( $aaa_chinese_block_editor_v2_ui ) {
		$single_post_continer_spacing = aaa_chinese_get_option( 'single-post-inside-spacing' );

		$container_lg_spacing = true === aaa_chinese_check_is_structural_setup() ? '3' : '6.67';

		$aaa_chinese_continer_left_spacing  = defined( 'AAA_EXT_VER' ) && aaa_chinese_responsive_spacing( $single_post_continer_spacing, 'left', 'desktop' ) ? aaa_chinese_responsive_spacing( $single_post_continer_spacing, 'left', 'desktop', $container_lg_spacing ) : 'var(--ast-container-default-xlg-padding)';
		$aaa_chinese_continer_right_spacing = defined( 'AAA_EXT_VER' ) && aaa_chinese_responsive_spacing( $single_post_continer_spacing, 'right', 'desktop' ) ? aaa_chinese_responsive_spacing( $single_post_continer_spacing, 'right', 'desktop', $container_lg_spacing ) : 'var(--ast-container-default-xlg-padding)';

		$alignwide_1200_left_negative_margin  = $aaa_chinese_continer_left_spacing ? 'calc(-1 * min(' . $aaa_chinese_continer_left_spacing . ', 20px))' : '-20px';
		$alignwide_1200_right_negative_margin = $aaa_chinese_continer_right_spacing ? 'calc(-1 * min(' . $aaa_chinese_continer_right_spacing . ', 20px))' : '-20px';

		$heading_width_comp          = AAA_Chinese_Dynamic_CSS::aaa_chinese_4_8_0_compatibility() ? 'none' : 'var(--wp--custom--ast-content-width-size)';
		$container_margin_left_comp  = AAA_Chinese_Dynamic_CSS::aaa_chinese_4_8_4_compatibility() ? 'auto' : 'calc(-1 * var(--wp--custom--ast-default-block-left-padding) )';
		$container_margin_right_comp = AAA_Chinese_Dynamic_CSS::aaa_chinese_4_8_4_compatibility() ? 'auto' : 'calc(-1 * var(--wp--custom--ast-default-block-right-padding) )';
		...
		$dynamic_css .= '
			...
			@media(min-width: 1201px) {
				.ast-separate-container .entry-content > .alignfull {
					margin-left: calc(-1 * ' . $aaa_chinese_continer_left_spacing . ' );
					margin-right: calc(-1 * ' . $aaa_chinese_continer_right_spacing . ' );
				}
	...
	$dynamic_css .= aaa_chinese_parse_css( $mobile_css, '', $mobile_breakpoint );
	/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
	...
	$lists_padding = AAA_Chinese_Dynamic_CSS::aaa_chinese_4_6_0_compatibility() ? 'padding-' . $direction . ': 20px;' : '';
