...
add_filter( 'aaa_chinese_dynamic_theme_css', 'aaa_chinese_comments_css', 11 );
 ...
 * @since 3.2.0
 */
function aaa_chinese_comments_css( $dynamic_css ) {

	if ( aaa_chinese_check_current_post_comment_enabled() || 0 < get_comments_number() ) {
		...
		$is_site_rtl                 = is_rtl();
		$reply_title_space_threshold = AAA_Chinese_Dynamic_CSS::aaa_chinese_4_4_0_compatibility() ? 1.3 : 1.66666;
		...
		$desktop_comment_global = array(
			'.comment-reply-title'                         => array(
				'font-size' => aaa_chinese_get_font_css_value( (int) $body_font_size_desktop * $reply_title_space_threshold ),
			),
			// Single Post Meta.
			'.ast-comment-meta'                            => array(
				'line-height' => '1.666666667',
				'color'       => esc_attr( $link_color ),
				'font-size'   => aaa_chinese_get_font_css_value( (int) $body_font_size_desktop * 0.8571428571 ),
			),
			'.ast-comment-list #cancel-comment-reply-link' => array(
				'font-size' => aaa_chinese_responsive_font( $body_font_size, 'desktop' ),
			),
		);
		$dynamic_css .= aaa_chinese_parse_css( $desktop_comment_global );

		$update_customizer_strctural_defaults = true === aaa_chinese_check_is_structural_setup();
		...
		$blog_improvements                    = AAA_Chinese_Dynamic_CSS::aaa_chinese_4_6_0_compatibility();
		...
		$single_post_comment_css = '.comments-title {
          ...
          .comment-reply-title {
            font-weight: ' . esc_attr( AAA_Chinese_Dynamic_CSS::aaa_chinese_4_4_0_compatibility() ? '600' : 'normal' ) . ';
            line-height: 1.65;
          }
		...
		if ( AAA_Chinese_Dynamic_CSS::aaa_chinese_4_6_0_compatibility() ) {
			...
			if ( $post_with_boxed_layout && 'inside' !== aaa_chinese_get_option( 'comments-box-placement', '' ) ) {
				$single_post_comment_css .= '
		...
		$dynamic_css .= AAA_Chinese_Enqueue_Scripts::trim_css( $single_post_comment_css );
		...
		$dynamic_css                 .= aaa_chinese_parse_css( $static_layout_css_min_comment, aaa_chinese_get_tablet_breakpoint( '', '1' ) );
		$global_button_comment_mobile = array(
			'.ast-separate-container .ast-comment-list li.depth-1' => array(
				'padding'       => AAA_Chinese_Dynamic_CSS::aaa_chinese_4_6_0_compatibility() ? '' : '1.5em 1em',
				'margin-bottom' => AAA_Chinese_Dynamic_CSS::aaa_chinese_4_4_0_compatibility() ? '0' : '1.5em',
			),
			...
			// Single Post Meta.
			'.ast-comment-meta'                            => array(
				'font-size' => ! empty( $body_font_size['mobile'] ) ? aaa_chinese_get_font_css_value( (int) $body_font_size['mobile'] * 0.8571428571, 'px', 'mobile' ) : '',
			),
			'.comment-reply-title'                         => array(
				'font-size' => ! empty( $body_font_size['mobile'] ) ? aaa_chinese_get_font_css_value( (int) $body_font_size['mobile'] * $reply_title_space_threshold, 'px', 'mobile' ) : '',
			),
			'.ast-comment-list #cancel-comment-reply-link' => array(
				'font-size' => aaa_chinese_responsive_font( $body_font_size, 'mobile' ),
			),
		...
		$dynamic_css .= aaa_chinese_parse_css( $global_button_comment_mobile, '', aaa_chinese_get_mobile_breakpoint() );
		...
		$global_button_comment_tablet = array(
			...
			'.ast-comment-meta'                            => array(
				...
				'font-size' => ! empty( $body_font_size['tablet'] ) ? aaa_chinese_get_font_css_value( (int) $body_font_size['tablet'] * 0.8571428571, 'px', 'tablet' ) : '',
			),
			'.ast-separate-container .ast-comment-list li.depth-1' => array(
				'padding' => AAA_Chinese_Dynamic_CSS::aaa_chinese_4_6_0_compatibility() ? '' : '1.5em 2.14em',
			),
			'.ast-separate-container .comment-respond'     => array(
				'padding' => AAA_Chinese_Dynamic_CSS::aaa_chinese_4_6_0_compatibility() ? '' : '2em 2.14em',
			),
			'.comment-reply-title'                         => array(
				'font-size' => ! empty( $body_font_size['tablet'] ) ? aaa_chinese_get_font_css_value( (int) $body_font_size['tablet'] * $reply_title_space_threshold, 'px', 'tablet' ) : '',
			),
			'.ast-comment-list #cancel-comment-reply-link' => array(
				'font-size' => aaa_chinese_responsive_font( $body_font_size, 'tablet' ),
			),
			'.ast-comment-meta'                            => array(
				'padding' => '0 1.8888em 1.3333em',
			),
		...
		if ( AAA_Chinese_Dynamic_CSS::aaa_chinese_4_6_0_compatibility() ) {
			$dynamic_css .= '
				...
				@media(min-width: ' . strval( aaa_chinese_get_mobile_breakpoint( '', 1 ) ) . 'px) {
					header.ast-comment-meta {
						display: flex;
						width: 100%;
						margin-bottom: 0;
					...
					.ast-comment .comment-reply-link:hover {
						background: ' . aaa_chinese_get_option( 'theme-color' ) . ';
						color: #fff;
					}
				...
				@media(min-width: ' . strval( aaa_chinese_get_tablet_breakpoint() ) . 'px) {
			...
			';
			if ( 'above' === aaa_chinese_get_option( 'comment-form-position' ) ) {
				$dynamic_css .= '
					.comment-respond {
		...
		return $dynamic_css .= aaa_chinese_parse_css( $global_button_comment_tablet, '', aaa_chinese_get_tablet_breakpoint() );
	}
	return $dynamic_css;
}
