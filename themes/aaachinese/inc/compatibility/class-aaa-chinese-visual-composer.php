...
/**
 * AAA Chinese Visual Composer Compatibility
 */
if ( ! class_exists( 'AAA_Chinese_Visual_Composer' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class AAA_Chinese_Visual_Composer {
		/**
		 * Member Variable
		 *
		 * @var object instance
		...
		public function vc_update_meta_setting( $id ) {
			...
			if ( false === aaa_chinese_enable_page_builder_compatibility() || 'post' === get_post_type() ) {
				return;
			}

			update_post_meta( $id, '_aaa_chinese_content_layout_flag', 'disabled' );
		...
		public function vc_frontend_default_setting() {
			...
			global $post;
			$id                = aaa_chinese_get_post_id();
			$page_builder_flag = get_post_meta( $id, '_aaa_chinese_content_layout_flag', true );
 ...
 */
AAA_Chinese_Visual_Composer::get_instance();
