...
/**
 * AAA Chinese WooCommerce Compatibility
 */
if ( ! class_exists( 'AAA_Chinese_Woocommerce' ) ) {
	...
	class AAA_Chinese_Woocommerce {
		...
		public function __construct() {
			...
			require_once AAA_THEME_DIR . 'inc/compatibility/woocommerce/woocommerce-common-functions.php';// phpcs:ignore: WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
			...
			add_filter( 'aaa_chinese_get_sidebar', array( $this, 'replace_store_sidebar' ) );
			// Store Sidebar Layout.
			add_filter( 'aaa_chinese_page_layout', array( $this, 'store_sidebar_layout' ) );
			// Store Content Layout.
			add_filter( 'aaa_chinese_get_content_layout', array( $this, 'store_content_layout' ) );
			...
			add_filter( 'aaa_chinese_get_dynamic_header_content', array( $this, 'aaa_chinese_header_cart' ), 10, 3 );

			// Add Cart option in dropdown.
			add_filter( 'aaa_chinese_header_section_elements', array( $this, 'header_section_elements' ) );
			...
			add_filter( 'woocommerce_get_stock_html', 'aaa_chinese_woo_product_in_stock', 10, 2 );

			add_filter( 'aaa_chinese_schema_body', array( $this, 'remove_body_schema' ) );

			// Header Cart Icon.
			add_action( 'aaa_chinese_woo_header_cart_icons_before', array( $this, 'header_cart_icon_markup' ) );

			add_action( 'aaa_chinese_cart_in_menu_class', array( $this, 'header_cart_icon_class' ), 99 );

			// WooCommerce Store Notice.
			add_filter( 'woocommerce_demo_store', array( $this, 'aaa_chinese_woocommerce_update_store_notice_atts' ) );

			add_filter( 'aaa_chinese_dynamic_theme_css', array( $this, 'aaa_chinese_woocommerce_store_dynamic_css' ) );

			// Single Product Free shipping.
			add_action( 'aaa_chinese_woo_single_price_after', array( $this, 'woocommerce_shipping_text' ) );
			...
			add_action( 'admin_bar_menu', array( $this, 'aaa_chinese_update_customize_admin_bar_link' ), 45 );
			...
			if ( ! defined( 'AAA_EXT_VER' ) || ( defined( 'AAA_EXT_VER' ) && ! AAA_Chinese_Ext_Extension::is_active( 'woocommerce' ) ) ) {
		...
		/**
		 * As WooCommerce-AAA Chinese pro options moved to theme, decide here to load from theme's end after 3.9.2 version.
		 *
		 * @since 3.9.2
		 * @return bool true|false.
		 */
		public static function load_theme_side_woocommerce_strcture() {
			return ! defined( 'AAA_EXT_VER' ) || aaa_chinese_addon_check_version( '3.9.2', '>=' );
		}
		...
		public function post_class( $classes ) {
			...
			if ( is_shop() || is_product_taxonomy() || ( post_type_exists( 'product' ) && 'product' === get_post_type() ) ) {
				...
				$shop_product_alignment = aaa_chinese_get_option( 'shop-product-align-responsive' );
		...
		public function modern_add_to_cart() {
			...
			// Product link markup.
			$header_woo_cart = aaa_chinese_get_option( 'woo-header-cart-icon', 'default' );
			$cart_icon       = true === AAA_Chinese_Icons::is_svg_icons() ? AAA_Chinese_Icons::get_icons( 'default' === $header_woo_cart ? 'bag' : $header_woo_cart ) : AAA_Chinese_Builder_UI_Controller::fetch_svg_icon( 'shopping-' . $header_woo_cart, false );
			$classes         = implode(
				' ',
				array_filter(
					array(
		...
		public function add_modern_triggers_on_image() {
			...
			if ( aaa_chinese_is_shop_page_modern_style() ) {
				...
				$html = apply_filters( 'aaa_chinese_addon_shop_cards_buttons_html', $markup, $product );
				/** @psalm-suppress TooManyArguments */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		...
		public function get_sale_flash_markup( $sale_notification, $product ) {
			...
			$classes = array();
			/** @psalm-suppress UndefinedFunction  */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			$classes[] = aaa_chinese_is_shop_page_modern_style() ? 'ast-on-card-button ast-onsale-card' : 'onsale';
			/** @psalm-suppress UndefinedFunction  */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			$classes = implode( ' ', $classes );

			// Generate markup.
			return '<span  ' . aaa_chinese_attr(
		...
		public function sale_flash( $markup, $post, $product ) {
			...
			/** @psalm-suppress UndefinedFunction  */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			if ( ( ! is_singular( 'product' ) && aaa_chinese_is_shop_page_modern_style() ) ) {
				/** @psalm-suppress UndefinedFunction  */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				return '';
			}
		...
		public function change_cart_close_icon( $string ) {
			return preg_replace_callback(
				...
				static function( $matches ) {
					return $matches[1] . AAA_Chinese_Builder_UI_Controller::fetch_svg_icon( 'close', false ) . $matches[3];
				},
				$string
			);
		}
		...
		public function store_widgets_dynamic() {
			...
			if ( aaa_chinese_has_pro_woocommerce_addon() && aaa_chinese_get_option( 'shop-filter-accordion' ) ) {
				$shop_filter_array['before_title']   = '<h2 class="widget-title">';
				$shop_filter_array['after_title']    = AAA_Chinese_Builder_UI_Controller::fetch_svg_icon( 'angle-down', false ) . '</h2>';
			...
			register_sidebar(
				apply_filters(
					'aaa_chinese_woocommerce_shop_sidebar_init',
					$shop_filter_array
				)
			);
		}
		...
		/**
		 * Update WooCommerce store notice. Extending this function to add custom data-attr as per AAA Chinese's configuration.
		 *
		 * @since 3.9.0
		 *
		 ...
		 */
		public function aaa_chinese_woocommerce_update_store_notice_atts( $notice ) {

			$store_notice_position = aaa_chinese_get_option( 'store-notice-position' );
		...
		public function woocommerce_shipping_text() {
			if ( aaa_chinese_get_option( 'single-product-enable-shipping' ) ) {
				$shipping_text = aaa_chinese_get_i18n_option( 'single-product-shipping-text', _x( '%astra%', 'WooCommerce Single Product: Shipping Text', 'astra' ), false );
		 ...
		 */
		public function aaa_chinese_woocommerce_store_dynamic_css( $dynamic_css, $dynamic_css_filtered = '' ) {
			...
			$store_notice_color    = aaa_chinese_get_option( 'store-notice-text-color' );
			$store_notice_bg_color = aaa_chinese_get_option( 'store-notice-background-color' );
			/**
			...
			if ( ! $notice_hidden && 'hang-over-top' === aaa_chinese_get_option( 'store-notice-position' ) ) {
			...
			$dynamic_css .= aaa_chinese_parse_css( $css_output_desktop );
			...
			if ( is_user_logged_in() ) {
				...
				$dynamic_css .= aaa_chinese_parse_css( $admin_bar_desktop_css, '783' );
				...
				$dynamic_css .= aaa_chinese_parse_css( $admin_bar_responsive_css, '', '782' );
			}
			return $dynamic_css;
		}
		...
		public function svg_cart_icon( $cart_total_label_position, $cart_label_markup, $cart_info_markup, $cart_icon ) {
			...
			add_filter( 'aaa_chinese_woo_default_header_cart_icon', '__return_false' );
		...
		public function header_cart_icon_markup() {
			...
			$woo_cart_icon_new_user = aaa_chinese_get_option( 'astra-woocommerce-cart-icons-flag', true );
			/** @psalm-suppress DocblockTypeContradiction */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			if ( apply_filters( 'aaa_chinese_woocommerce_cart_icon', $woo_cart_icon_new_user ) ) {
				if ( false === AAA_Chinese_Builder_Helper::$is_header_footer_builder_active ) {
					return;
				}
			} else {
				if ( false === AAA_Chinese_Builder_Helper::$is_header_footer_builder_active && ! defined( 'AAA_EXT_VER' ) ) {
					return;
				}
			}

			$defaults           = apply_filters( 'aaa_chinese_woocommerce_cart_icon', $woo_cart_icon_new_user ) ? 'bag' : 'default';
			$icon               = aaa_chinese_get_option( 'woo-header-cart-icon', $defaults );
			$cart_count_display = apply_filters( 'aaa_chinese_header_cart_count', true );
			$cart_title         = apply_filters( 'aaa_chinese_header_cart_title', __( 'Cart', 'astra' ) );

			$cart_title_markup         = '<span class="ast-woo-header-cart-title">' . esc_html( $cart_title ) . '</span>';
			$cart_total_label_position = aaa_chinese_get_option( 'woo-header-cart-icon-total-label-position' );
			$cart_total_markup         = '';
			...
			$cart_check_total = aaa_chinese_get_option( 'woo-header-cart-total-label' ) && null !== WC()->cart ? intval( WC()->cart->get_cart_contents_total() ) > 0 : true;
			/** @psalm-suppress RedundantConditionGivenDocblockType */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			...
			$display_cart_label = aaa_chinese_get_i18n_option( 'woo-header-cart-label-display', _x( '%astra%', 'Header Builder: Cart Widget - Cart Label', 'astra' ) );
			...
			$cart_icon = sprintf(
				...
				$icon ? ( false !== AAA_Chinese_Icons::is_svg_icons() ? AAA_Chinese_Icons::get_icons( $icon ) : '' ) : ''
			);
		...
		public function header_cart_icon_class( $classes ) {
			...
			$header_cart_icon_style = aaa_chinese_get_option( 'woo-header-cart-icon-style' );

			$classes[]                  = 'ast-menu-cart-' . $header_cart_icon_style;
			$header_cart_icon_has_color = aaa_chinese_get_option( 'woo-header-cart-icon-color' );
		 ...
		 * WooCommerce adds it's own product schema hence schema data from AAA Chinese should be disabled here.
		 *
		 * @since 1.8.0
		...
		public function rating_markup( $html, $rating, $count ) {
			...
			if ( $rating > 0 ) {
				...
				if ( $review_count > 0 && aaa_chinese_get_option( 'shop-ratings-product-archive' ) === 'count_string' && ! is_product() ) {
		...
		public function cart_page_upselles() {
			...
			$upselles_enabled = aaa_chinese_get_option( 'enable-cart-upsells' );
			if ( ! $upselles_enabled ) {
		...
		public function woo_filter_style( $styles ) {
			...
			if ( ! AAA_Chinese_Enqueue_Scripts::should_load_woocommerce_css() ) {
				return array();
			}
			...
			$css_uri = AAA_THEME_URI . 'assets/css/' . $dir_name . '/compatibility/woocommerce/';

			// Register & Enqueue Styles.
			// Generate CSS URL.

			if ( ! AAA_Chinese_Builder_Helper::apply_flex_based_css() ) {
				$styles = array(
					'woocommerce-layout'      => array(
						'src'     => $css_uri . 'woocommerce-layout' . $file_prefix . '.css',
						'deps'    => '',
						'version' => AAA_THEME_VERSION,
						'media'   => 'all',
						'has_rtl' => true,
					),
					'woocommerce-smallscreen' => array(
						...
						'version' => AAA_THEME_VERSION,
						'media'   => 'only screen and (max-width: ' . apply_filters( 'woocommerce_style_smallscreen_breakpoint', aaa_chinese_get_tablet_breakpoint() . 'px' ) . ')', // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
						'has_rtl' => true,
					),
		...
		public function theme_defaults( $defaults ) {
			...
			$defaults['single-product-plus-minus-button'] = aaa_chinese_has_pro_woocommerce_addon() ? true : false;
		...
		public function shop_columns( $col ) {
			...
			$aaa_chinese_shop_col = aaa_chinese_get_option(
				'shop-grids',
				array(
					'desktop' => 4,
					'tablet'  => 3,
			...
			return $aaa_chinese_shop_col['desktop'];
		}
		 ...
		 * @return bool
		 */
		public function aaa_chinese_woo_is_subcategory( $category_id = null ) {
		...
		public function shop_no_of_products() {
			...
			if ( is_product_taxonomy() && 'subcategories' === $taxonomy_page_display ) {
				$products = aaa_chinese_get_option( 'shop-no-of-products' );
				if ( $this->aaa_chinese_woo_is_subcategory() ) {
					return $products;
				}
		...
		public function shop_page_products_item_class( $classes = '' ) {
			...
			if ( is_shop() || is_product_taxonomy() ) {
				$shop_grid = aaa_chinese_get_option(
					'shop-grids',
					array(
						'desktop' => 4,
						'tablet'  => 3,
			...
			$rt_section = aaa_chinese_get_option( 'header-main-rt-section' );

			if ( 'woocommerce' === $rt_section ) {
				$classes[] = 'ast-woocommerce-cart-menu';
			}

			if ( is_store_notice_showing() && 'hang-over-top' === aaa_chinese_get_option( 'store-notice-position' ) ) {
		...
		public function get_grid_column_count( $type = 'archive', $device = 'desktop', $default = 2 ) {
			...
			if ( 'archive' === $type ) {
				$products_grid = aaa_chinese_get_option(
					'shop-grids',
					array(
						'desktop' => 4,
						'tablet'  => 3,
				...
				$products_grid = aaa_chinese_get_option( 'single-product-related-upsell-grid' );
			}
		...
		public function store_widgets_init() {
			...
			register_sidebar(
				apply_filters(
					'aaa_chinese_woocommerce_single_sidebar_init',
					array(
		...
		public function store_sidebar_layout( $sidebar_layout ) {
			...
			if ( is_shop() || is_product_taxonomy() || is_checkout() || is_cart() || is_account_page() || is_product() ) {
				...
				$woo_sidebar                 = aaa_chinese_get_option( 'woocommerce-sidebar-layout' );
				$aaa_chinese_with_modern_ecommerce = aaa_chinese_get_option( 'modern-ecommerce-setup', true );
				...
				if ( is_shop() || is_product_taxonomy() ) {
					$global_page_specific_layout = aaa_chinese_get_option( 'archive-product-sidebar-layout', 'default' );
				}

				if ( is_product() ) {
					$single_product_fallback_sidebar = false === $aaa_chinese_with_modern_ecommerce ? aaa_chinese_get_option( 'site-sidebar-layout' ) : aaa_chinese_get_option( 'woocommerce-sidebar-layout' );
					$single_product_sidebar          = aaa_chinese_get_option( 'single-product-sidebar-layout', 'default' );
				...
				if ( is_shop() ) {
					...
					$shop_sidebar = aaa_chinese_get_option_meta( 'site-sidebar-layout', '', true );
				}
			...
			return apply_filters( 'aaa_chinese_get_store_sidebar_layout', $sidebar_layout );
		}
		/**
		 * WooCommerce Container
		 *
		...
		public function store_content_layout( $layout ) {
			...
			if ( is_woocommerce() || is_checkout() || is_cart() || is_account_page() ) {
				...
				$woo_layout = aaa_chinese_toggle_layout( 'woocommerce-ast-content-layout', 'global', false );
				...
				if ( is_shop() || is_product_taxonomy() ) {
					$global_page_specific_layout = aaa_chinese_toggle_layout( 'archive-product-ast-content-layout', 'archive', false );
				}

				if ( is_product() ) {
					$global_page_specific_layout = aaa_chinese_toggle_layout( 'single-product-ast-content-layout', 'single', false );
				}
				...
				if ( is_shop() ) {
					$shop_page_id = get_option( 'woocommerce_shop_page_id' );
					$shop_layout  = aaa_chinese_toggle_layout( 'ast-site-content-layout', 'meta', $shop_page_id );
				} elseif ( is_product_taxonomy() ) {
					$shop_layout = 'default';
				} else {
					$old_meta_layout = aaa_chinese_get_option_meta( 'site-content-layout', '', true );
					if ( isset( $old_meta_layout ) ) {
						$shop_layout = aaa_chinese_toggle_layout( 'ast-site-content-layout', 'meta', false, $old_meta_layout );
					} else {
						$shop_layout = aaa_chinese_toggle_layout( 'ast-site-content-layout', 'meta', false );
					}
				}
			...
			return apply_filters( 'aaa_chinese_get_store_content_layout', $layout );
		}
		...
		public function shop_meta_option() {
			...
			if ( is_shop() ) {
				...
				if ( 'disabled' === $main_header_display ) {
					remove_action( 'aaa_chinese_masthead', 'aaa_chinese_masthead_primary_template' );
				}

				if ( 'disabled' === $footer_layout ) {
					remove_action( 'aaa_chinese_footer_content', 'aaa_chinese_footer_small_footer_template', 5 );
				}
			}
		}
		...
		public function shop_customization() {
			...
			if ( ! apply_filters( 'aaa_chinese_woo_shop_product_structure_override', false ) ) {

				add_action( 'woocommerce_before_shop_loop_item', 'aaa_chinese_woo_shop_thumbnail_wrap_start', 6 );
				/**
				 * Add sale flash before shop loop.
				 */
				add_action( 'woocommerce_before_shop_loop_item', 'woocommerce_show_product_loop_sale_flash', 9 );

				add_action( 'woocommerce_after_shop_loop_item', 'aaa_chinese_woo_shop_thumbnail_wrap_end', 8 );
				/**
				 * Add Out of Stock to the Shop page
				 */
				add_action( 'woocommerce_shop_loop_item_title', 'aaa_chinese_woo_shop_out_of_stock', 8 );
				 ...
				 */
				add_action( 'woocommerce_after_shop_loop_item', 'aaa_chinese_woo_woocommerce_shop_product_content' );
			}
		}
		...
		public function single_product_customization() {
			...
			if ( aaa_chinese_get_option( 'single-product-breadcrumb-disable' ) ) {
		...
		public function woocommerce_init() {
			add_action( 'woocommerce_after_mini_cart', array( $this, 'aaa_chinese_update_flyout_cart_layout' ) );
		...
		public function before_main_content_start() {
			$site_sidebar = aaa_chinese_page_layout();
			...
			<div id="primary" class="content-area primary">
				...
				<?php aaa_chinese_primary_content_top(); ?>
		...
		public function before_main_content_end() {
			?>
				...
				<?php aaa_chinese_primary_content_bottom(); ?>
		...
		/**
		 * AAA Chinese update default font size and font weight.
		 *
		 * @since 4.6.0
		 * @return bool
		 */
		public static function aaa_chinese_update_default_font_styling() {
			$aaa_chinese_settings = aaa_chinese_get_options();
			return apply_filters( 'aaa_chinese_default_font_style_update', isset( $aaa_chinese_settings['ast-font-style-update'] ) ? false : true );
		}
		...
		public function add_scripts_styles() {
			if ( ! AAA_Chinese_Enqueue_Scripts::should_load_woocommerce_js() ) {
				return;
			}
			 ...
			 */
			$is_site_rtl                             = is_rtl();
			$theme_color                             = aaa_chinese_get_option( 'theme-color' );
			$link_color                              = aaa_chinese_get_option( 'link-color', $theme_color );
			$text_color                              = aaa_chinese_get_option( 'text-color' );
			$link_h_color                            = aaa_chinese_get_option( 'link-h-color' );
			$if_free_shipping                        = aaa_chinese_get_option( 'single-product-enable-shipping' );
			$single_product_heading_tab_active_color = aaa_chinese_get_option( 'single-product-heading-tab-active-color' );
			$global_palette                          = aaa_chinese_get_option( 'global-color-palette' );
			$ltr_left                                = $is_site_rtl ? 'right' : 'left';
			$ltr_right                               = $is_site_rtl ? 'left' : 'right';
			$icon_cart_color_slug                    = '';
			$theme_btn_font_size                     = aaa_chinese_get_option( 'font-size-button' );
			$font_style_updates                      = self::aaa_chinese_update_default_font_styling();
			...
			$can_update_cart_color   = aaa_chinese_cart_color_default_icon_old_header();
			$cart_new_color_setting  = aaa_chinese_get_option( $icon_cart_color_slug, $theme_color );
			$header_cart_count_color = $can_update_cart_color ? $cart_new_color_setting : $theme_color;

			$btn_color = aaa_chinese_get_option( 'button-color' );
			if ( empty( $btn_color ) ) {
				$btn_color = aaa_chinese_get_foreground_color( $theme_color );
			}

			$btn_h_color = aaa_chinese_get_option( 'button-h-color' );
			if ( empty( $btn_h_color ) ) {
				$btn_h_color = aaa_chinese_get_foreground_color( $link_h_color );
			}
			$btn_bg_color   = aaa_chinese_get_option( 'button-bg-color', '', $theme_color );
			$btn_bg_h_color = aaa_chinese_get_option( 'button-bg-h-color', '', $link_h_color );

			$btn_border_radius_fields = aaa_chinese_get_option( 'button-radius-fields' );
			$theme_btn_padding        = aaa_chinese_get_option( 'theme-button-padding' );

			$cart_h_color = aaa_chinese_get_foreground_color( $link_h_color );

			$site_content_width         = aaa_chinese_get_option( 'site-content-width', 1200 );
			$woo_shop_archive_width     = aaa_chinese_get_option( 'shop-archive-width' );
			$woo_shop_archive_max_width = aaa_chinese_get_option( 'shop-archive-max-width' );

			// global button border settings.
			$global_custom_button_border_size = aaa_chinese_get_option( 'theme-button-border-group-border-size' );
			$btn_border_color                 = aaa_chinese_get_option( 'theme-button-border-group-border-color' );
			$btn_border_h_color               = aaa_chinese_get_option( 'theme-button-border-group-border-h-color' );
			...
			$aaa_chinese_support_woo_btns_global = AAA_Chinese_Dynamic_CSS::aaa_chinese_woo_support_global_settings();

			if ( $aaa_chinese_support_woo_btns_global ) {
				$woo_btn_compatibility_desktop = aaa_chinese_get_font_array_css( aaa_chinese_get_option( 'font-family-button' ), aaa_chinese_get_option( 'font-weight-button' ), $theme_btn_font_size, 'font-extras-button' );
				$woo_btn_compatibility_tablet  = array(
					'font-size' => aaa_chinese_get_font_css_value( $theme_btn_font_size['tablet'], $theme_btn_font_size['tablet-unit'] ),
				);
				$woo_btn_compatibility_mobile  = array(
					'font-size' => aaa_chinese_get_font_css_value( $theme_btn_font_size['mobile'], $theme_btn_font_size['mobile-unit'] ),
				);
			}
			...
			$css_desktop_output = array(
				...
				'.woocommerce span.onsale, .wc-block-grid__product .wc-block-grid__product-onsale' => array(
					...
					'color'            => aaa_chinese_get_foreground_color( $theme_color ),
				),
			...
			if ( ! aaa_chinese_wc_is_star_rating_compatibility() ) {
			...
			if ( ! $aaa_chinese_support_woo_btns_global ) {
				...
				$css_desktop_output['.woocommerce-js a.button, .woocommerce button.button, .woocommerce .woocommerce-message a.button, .woocommerce #respond input#submit.alt, .woocommerce-js a.button.alt, .woocommerce button.button.alt, .woocommerce input.button.alt, .woocommerce input.button,.woocommerce-cart table.cart td.actions .button, .woocommerce form.checkout_coupon .button, .woocommerce #respond input#submit, .wc-block-grid__products .wc-block-grid__product .wp-block-button__link']               = array_merge(
					...
					array(
						'border-top-left-radius'     => aaa_chinese_responsive_spacing( $btn_border_radius_fields, 'top', 'desktop' ),
						'border-top-right-radius'    => aaa_chinese_responsive_spacing( $btn_border_radius_fields, 'right', 'desktop' ),
						'border-bottom-right-radius' => aaa_chinese_responsive_spacing( $btn_border_radius_fields, 'bottom', 'desktop' ),
						'border-bottom-left-radius'  => aaa_chinese_responsive_spacing( $btn_border_radius_fields, 'left', 'desktop' ),
						'padding-top'                => aaa_chinese_responsive_spacing( $theme_btn_padding, 'top', 'desktop' ),
						'padding-right'              => aaa_chinese_responsive_spacing( $theme_btn_padding, 'right', 'desktop' ),
						'padding-bottom'             => aaa_chinese_responsive_spacing( $theme_btn_padding, 'bottom', 'desktop' ),
						'padding-left'               => aaa_chinese_responsive_spacing( $theme_btn_padding, 'left', 'desktop' ),
					)
				);
			...
			if ( AAA_Chinese_Dynamic_CSS::v4_block_editor_compat() ) {
			...
			if ( is_cart() && false === AAA_Chinese_Builder_Helper::apply_flex_based_css() && true === aaa_chinese_get_option( 'cart-modern-layout' ) && true === aaa_chinese_get_option( 'enable-cart-upsells' ) ) {
				$css_desktop_output['.woocommerce[class*="rel-up-columns-"] .site-main div.product .related.products ul.products li.product, .woocommerce-page .site-main ul.products li.product'] = array(
					'width' => '100%',
				);
			}
			...
			if ( apply_filters( 'aaa_chinese_theme_woocommerce_global_h2_font', aaa_chinese_get_option( 'woo_support_global_settings', false ) ) ) {
			...
			if ( is_account_page() && false === aaa_chinese_get_option( 'modern-woo-account-view', false ) ) {
				$css_output .= '
			...
			if ( 'flyout' === aaa_chinese_get_option( 'woo-header-cart-click-action' ) ) {
				$css_output .= '.woocommerce a.added_to_cart { display: none; }';
			}

			/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			if ( ! ( defined( 'AAA_EXT_VER' ) && class_exists( 'AAA_Chinese_Ext_Extension' ) && AAA_Chinese_Ext_Extension::is_active( 'woocommerce' ) ) ) {
				/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				$css_output .= '
			...
			if ( true === aaa_chinese_check_is_structural_setup() ) {
			...
			$css_output .= aaa_chinese_parse_css( $css_desktop_output );
			...
			$css_output .= aaa_chinese_parse_css( $tablet_css_shop_page_grid, aaa_chinese_get_mobile_breakpoint( '', 1 ), aaa_chinese_get_tablet_breakpoint() );
			...
			if ( ! AAA_Chinese_Builder_Helper::apply_flex_based_css() ) {
				...
				$css_output .= aaa_chinese_parse_css( $tablet_shop_page_grid_lang_direction_css, aaa_chinese_get_mobile_breakpoint( '', 1 ), aaa_chinese_get_tablet_breakpoint() );
			}
			...
			$css_output .= aaa_chinese_parse_css( $min_tablet_css, aaa_chinese_get_tablet_breakpoint( '', 1 ) );
			...
			if ( ! $aaa_chinese_support_woo_btns_global ) {
				$css_global_button_tablet['.woocommerce-js a.button, .woocommerce button.button, .woocommerce .woocommerce-message a.button, .woocommerce #respond input#submit.alt, .woocommerce-js a.button.alt, .woocommerce button.button.alt, .woocommerce input.button.alt, .woocommerce input.button,.woocommerce-cart table.cart td.actions .button, .woocommerce form.checkout_coupon .button, .woocommerce #respond input#submit, .wc-block-grid__products .wc-block-grid__product .wp-block-button__link'] = array_merge(
					...
					array(
						'border-top-left-radius'     => aaa_chinese_responsive_spacing( $btn_border_radius_fields, 'top', 'tablet' ),
						'border-top-right-radius'    => aaa_chinese_responsive_spacing( $btn_border_radius_fields, 'right', 'tablet' ),
						'border-bottom-right-radius' => aaa_chinese_responsive_spacing( $btn_border_radius_fields, 'bottom', 'tablet' ),
						'border-bottom-left-radius'  => aaa_chinese_responsive_spacing( $btn_border_radius_fields, 'left', 'tablet' ),
						'padding-top'                => aaa_chinese_responsive_spacing( $theme_btn_padding, 'top', 'tablet' ),
						'padding-right'              => aaa_chinese_responsive_spacing( $theme_btn_padding, 'right', 'tablet' ),
						'padding-bottom'             => aaa_chinese_responsive_spacing( $theme_btn_padding, 'bottom', 'tablet' ),
						'padding-left'               => aaa_chinese_responsive_spacing( $theme_btn_padding, 'left', 'tablet' ),
					)
				);
			}
			...
			$css_output .= aaa_chinese_parse_css( $css_global_button_tablet, '', aaa_chinese_get_tablet_breakpoint() );
			...
			if ( ! $aaa_chinese_support_woo_btns_global ) {
				...
				$css_global_button_mobile['.woocommerce-js a.button, .woocommerce button.button, .woocommerce .woocommerce-message a.button, .woocommerce #respond input#submit.alt, .woocommerce-js a.button.alt, .woocommerce button.button.alt, .woocommerce input.button.alt, .woocommerce input.button,.woocommerce-cart table.cart td.actions .button, .woocommerce form.checkout_coupon .button, .woocommerce #respond input#submit, .wc-block-grid__products .wc-block-grid__product .wp-block-button__link'] = array_merge(
					...
					array(
						'border-top-left-radius'     => aaa_chinese_responsive_spacing( $btn_border_radius_fields, 'top', 'mobile' ),
						'border-top-right-radius'    => aaa_chinese_responsive_spacing( $btn_border_radius_fields, 'right', 'mobile' ),
						'border-bottom-right-radius' => aaa_chinese_responsive_spacing( $btn_border_radius_fields, 'bottom', 'mobile' ),
						'border-bottom-left-radius'  => aaa_chinese_responsive_spacing( $btn_border_radius_fields, 'left', 'mobile' ),
						'padding-top'                => aaa_chinese_responsive_spacing( $theme_btn_padding, 'top', 'mobile' ),
						'padding-right'              => aaa_chinese_responsive_spacing( $theme_btn_padding, 'right', 'mobile' ),
						'padding-bottom'             => aaa_chinese_responsive_spacing( $theme_btn_padding, 'bottom', 'mobile' ),
						'padding-left'               => aaa_chinese_responsive_spacing( $theme_btn_padding, 'left', 'mobile' ),
					)
				);
			}
			...
			$css_output .= aaa_chinese_parse_css( $css_global_button_mobile, '', aaa_chinese_get_mobile_breakpoint() );
			...
			$css_output .= aaa_chinese_parse_css( $global_button_mobile_lang_direction_css, '', aaa_chinese_get_mobile_breakpoint() );

			if ( 'page-builder' !== aaa_chinese_get_content_layout() ) {
				...
				if ( 'custom' === $woo_shop_archive_width ) {
					...
					$site_width  = array(
						'.ast-woo-shop-archive .site-content > .ast-container' => array(
							'max-width' => aaa_chinese_get_css_value( $woo_shop_archive_max_width, 'px' ),
						),
					);
					$css_output .= aaa_chinese_parse_css( $site_width, aaa_chinese_get_tablet_breakpoint( '', 1 ) );

				} else {
					// Woocommerce shop archive default width.
					$site_width = array(
						'.ast-woo-shop-archive .site-content > .ast-container' => array(
							'max-width' => aaa_chinese_get_css_value( $site_content_width + 40, 'px' ),
						),
					);
			...
			/* Parse CSS from array()*/
			$css_output .= aaa_chinese_parse_css( $woo_product_css, aaa_chinese_get_tablet_breakpoint( '', 1 ) );
			...
			*/
			if ( self::aaa_chinese_global_btn_woo_comp() ) {
				if ( ! $aaa_chinese_support_woo_btns_global ) {
					$woo_global_button_css = array(
						'.woocommerce-js a.button , .woocommerce button.button.alt ,.woocommerce-page table.cart td.actions .button, .woocommerce-page #content table.cart td.actions .button , .woocommerce-js a.button.alt ,.woocommerce .woocommerce-message a.button , .ast-site-header-cart .widget_shopping_cart .buttons .button.checkout, .woocommerce button.button.alt.disabled , .wc-block-grid__products .wc-block-grid__product .wp-block-button__link ' => array(
							'border'              => 'solid',
							'border-top-width'    => isset( $global_custom_button_border_size['top'] ) && '' !== $global_custom_button_border_size['top'] ? aaa_chinese_get_css_value( $global_custom_button_border_size['top'], 'px' ) : '0',
							'border-right-width'  => isset( $global_custom_button_border_size['right'] ) && '' !== $global_custom_button_border_size['right'] ? aaa_chinese_get_css_value( $global_custom_button_border_size['right'], 'px' ) : '0',
							'border-left-width'   => isset( $global_custom_button_border_size['left'] ) && '' !== $global_custom_button_border_size['left'] ? aaa_chinese_get_css_value( $global_custom_button_border_size['left'], 'px' ) : '0',
							'border-bottom-width' => isset( $global_custom_button_border_size['bottom'] ) && '' !== $global_custom_button_border_size['bottom'] ? aaa_chinese_get_css_value( $global_custom_button_border_size['bottom'], 'px' ) : '0',
					...
					$css_output           .= aaa_chinese_parse_css( $woo_global_button_css );
				}
				...
				if ( $if_free_shipping ) {
					...
					$css_output            .= aaa_chinese_parse_css( $woo_free_shipping_text );
				}
			}

			if ( ! is_shop() && ! is_product() ) {

				$css_output .= aaa_chinese_parse_css(
					array(
			...
			$css_output .= aaa_chinese_parse_css( $woo_product_lang_direction_css, aaa_chinese_get_tablet_breakpoint( '', 1 ) );
			/** @psalm-suppress InvalidScalarArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			/**
			 * Single page cart button size.
			 */
			$single_product_cart_button_width = aaa_chinese_get_option( 'single-product-cart-button-width' );
			...
			$css_output_cart_button_width_desktop = array(
				...
				$single_cart_button => array(
					'width' => aaa_chinese_get_css_value( $single_product_cart_button_width_desktop, '%' ),
				),
			);

			$css_output .= aaa_chinese_parse_css( $css_output_cart_button_width_desktop );

			$css_output_cart_button_width_mobile = array(

				$single_cart_button => array(
					'width' => aaa_chinese_get_css_value( $single_product_cart_button_width_mobile, '%' ),
				),
			);
			...
			$css_output_cart_button_width_tablet = array(
				...
				$single_cart_button => array(
					'width' => aaa_chinese_get_css_value( $single_product_cart_button_width_tablet, '%' ),
				),
			);
			/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			$css_output .= aaa_chinese_parse_css( $css_output_cart_button_width_tablet, '', aaa_chinese_get_tablet_breakpoint() );
			/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			$css_output .= aaa_chinese_parse_css( $css_output_cart_button_width_mobile, '', aaa_chinese_get_mobile_breakpoint() );
			/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			...
			$css_output .= aaa_chinese_parse_css( $css_output_woo_select_default );

			$is_sticky_add_to_cart_position_active = aaa_chinese_get_option( 'single-product-sticky-add-to-cart' );
			...
			if ( is_product() && $is_sticky_add_to_cart_position_active ) {
				 ...
				 */
				$sticky_add_to_cart_position         = aaa_chinese_get_option( 'single-product-sticky-add-to-cart-position' );
				$sticky_add_to_cart_text_color       = aaa_chinese_get_option( 'single-product-sticky-add-to-cart-text-color' );
				$sticky_add_to_cart_bg_color         = aaa_chinese_get_option( 'single-product-sticky-add-to-cart-bg-color' );
				$sticky_add_to_cart_btn_text_color   = aaa_chinese_get_option( 'single-product-sticky-add-to-cart-btn-n-color' );
				$sticky_add_to_cart_btn_text_color_h = aaa_chinese_get_option( 'single-product-sticky-add-to-cart-btn-h-color' );
				$sticky_add_to_cart_btn_bg_color     = aaa_chinese_get_option( 'single-product-sticky-add-to-cart-btn-bg-n-color' );
				$sticky_add_to_cart_btn_bg_color_h   = aaa_chinese_get_option( 'single-product-sticky-add-to-cart-btn-bg-h-color' );
				...
				if ( is_admin_bar_showing() ) {
					...
					$css_output .= aaa_chinese_parse_css( $sticky_add_to_cart_admin_bar, '601' );
				}

				/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				$css_output .= aaa_chinese_parse_css( $sticky_add_to_cart_responsive_tablet, '', aaa_chinese_get_tablet_breakpoint() );
				/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort

				/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				$css_output .= aaa_chinese_parse_css( $sticky_add_to_cart_responsive_mobile, '', aaa_chinese_get_mobile_breakpoint() );
				/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort

				$css_output .= aaa_chinese_parse_css( $sticky_add_to_cart_p );
				$css_output .= aaa_chinese_parse_css( $sticky_add_to_cart );
			}

			$aaa_chinese_add_to_cart_quantity_btn_enabled = aaa_chinese_add_to_cart_quantity_btn_enabled();

			// Add to cart quantity button.
			if ( $aaa_chinese_add_to_cart_quantity_btn_enabled ) {
			...
			if ( 'shop-page-modern-style' === aaa_chinese_get_option( 'shop-style' ) ) {
				$modern_shop_page_css = '';

				if ( 'none' !== aaa_chinese_get_option( 'product-sale-notification', 'default' ) ) {
					$modern_shop_page_css .= '
						.ast-onsale-card {
							position: absolute;
							top: 1.5em;
			...
			if ( self::load_theme_side_woocommerce_strcture() ) {
				$css_output .= $this->aaa_chinese_shop_summary_box_alignment();
			}
			...
			$woo_variation_layout = aaa_chinese_get_option( 'single-product-variation-tabs-layout' );
			...
			if ( 'horizontal' === $woo_variation_layout ) {
				...
				$css_output .= aaa_chinese_parse_css( $css_output_woo_variation_layout );
			}
			...
			$css_output .= aaa_chinese_parse_css( $woo_active_filter_css );

			// Single product payment.
			$single_product_payment_array = aaa_chinese_get_option( 'single-product-structure' );
			...
			if (
				aaa_chinese_load_woocommerce_login_form_password_icon()
				&& (
					is_account_page() ||
					(
						is_checkout() &&
				...
				$css_output                   .= aaa_chinese_parse_css( $css_output_show_password_icon );
			}

			wp_add_inline_style( 'woocommerce-general', apply_filters( 'aaa_chinese_theme_woocommerce_dynamic_css', $css_output ) );
			...
			/* Parse CSS from array() */
			$yith_wcwl_main_style = aaa_chinese_parse_css( $yith_wcwl_main_style );
			...
			/* Parse CSS from array()*/
			/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			$yith_wcwl_main_style .= aaa_chinese_parse_css( $yith_wcwl_main_style_small, '', aaa_chinese_get_tablet_breakpoint() );
			/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		 ...
		 * @return string
		 */
		public function aaa_chinese_shop_summary_box_alignment() {
			$shop_product_alignment = aaa_chinese_get_option( 'shop-product-align-responsive' );
			$desktop_alignment      = isset( $shop_product_alignment['desktop'] ) ? $shop_product_alignment['desktop'] : '';
			$tablet_alignment       = isset( $shop_product_alignment['tablet'] ) ? $shop_product_alignment['tablet'] : '';
			$mobile_alignment       = isset( $shop_product_alignment['mobile'] ) ? $shop_product_alignment['mobile'] : '';
		...
		public function customize_register( $wp_customize ) {
			 ...
			 */
			require AAA_THEME_DIR . 'inc/compatibility/woocommerce/customizer/class-aaa-chinese-customizer-register-woo-section.php';

			/**
			 * Sections
			 */
			require AAA_THEME_DIR . 'inc/compatibility/woocommerce/customizer/sections/class-aaa-chinese-woo-shop-container-configs.php';
			require AAA_THEME_DIR . 'inc/compatibility/woocommerce/customizer/sections/class-aaa-chinese-woo-shop-sidebar-configs.php';
			require AAA_THEME_DIR . 'inc/compatibility/woocommerce/customizer/sections/layout/class-aaa-chinese-woo-shop-layout-configs.php';
			require AAA_THEME_DIR . 'inc/compatibility/woocommerce/customizer/sections/layout/class-aaa-chinese-woo-shop-single-layout-configs.php';
			require AAA_THEME_DIR . 'inc/compatibility/woocommerce/customizer/sections/layout/class-aaa-chinese-woo-shop-cart-layout-configs.php';
			require AAA_THEME_DIR . 'inc/compatibility/woocommerce/customizer/sections/layout/class-aaa-chinese-woo-shop-misc-layout-configs.php';
			require AAA_THEME_DIR . 'inc/compatibility/woocommerce/customizer/sections/class-aaa-chinese-woo-store-notice-configs.php';
		 ...
		 * @since 1.0.0
		 */
		public function aaa_chinese_header_cart( $output, $section, $section_type ) {

			if ( 'woocommerce' === $section_type && apply_filters( 'aaa_chinese_woo_header_cart_icon', true ) ) {
		...
		public function woo_mini_cart_markup( $device = 'desktop' ) {
			...
			$cart_click_action   = aaa_chinese_get_option( 'woo-header-cart-click-action', 'default' );
			$desktop_cart_flyout = 'flyout' === $cart_click_action ? 'ast-desktop-cart-flyout' : '';
			$cart_menu_classes   = apply_filters( 'aaa_chinese_cart_in_menu_class', array( 'ast-menu-cart-with-border', $desktop_cart_flyout ) );
			ob_start();
			if ( is_customize_preview() && true === AAA_Chinese_Builder_Helper::$is_header_footer_builder_active ) {
				AAA_Chinese_Builder_UI_Controller::render_customizer_edit_button();
			}
			?>
			<div class="ast-site-header-cart <?php echo esc_attr( implode( ' ', $cart_menu_classes ) ); ?>">
				<div class="ast-site-header-cart-li <?php echo esc_attr( $class ); ?>">
					<?php $this->aaa_chinese_get_cart_link(); ?>
				</div>
		 ...
		 * @since  1.0.0
		 */
		public function aaa_chinese_get_cart_link() {
			// Suppress TooFewArguments warning for this line
			/** @psalm-suppress TooFewArguments */
			$woo_cart_link = aaa_chinese_woocommerce_get_cart_url();
			/** @psalm-suppress RedundantConditionGivenDocblockType */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			...
			<a href="<?php echo esc_url( $woo_cart_link ); ?>" class="cart-container ast-cart-desktop-position-<?php echo esc_attr( $cart_total_label_position['desktop'] ); ?> ast-cart-mobile-position-<?php echo esc_attr( $cart_total_label_position['mobile'] ); ?> ast-cart-tablet-position-<?php echo esc_attr( $cart_total_label_position['tablet'] ); ?>" aria-label="<?php echo esc_attr( $aria_label ); ?>">
						...
						<?php
						do_action( 'aaa_chinese_woo_header_cart_icons_before' );

						if ( apply_filters( 'aaa_chinese_woo_default_header_cart_icon', true ) ) {
							?>
							<div class="ast-cart-menu-wrap">
								<span class="count">
								<span class="ast-count-text">
									<?php
									if ( apply_filters( 'aaa_chinese_woo_header_cart_total', true ) && null != WC()->cart ) {
						...
						do_action( 'aaa_chinese_woo_header_cart_icons_after' );
		...
		public function cart_link_fragment( $fragments ) {
			...
			add_filter( 'woocommerce_get_cart_url', 'aaa_chinese_woocommerce_get_cart_url' );

			ob_start();
			$this->aaa_chinese_get_cart_link();
			...
			remove_filter( 'woocommerce_get_cart_url', 'aaa_chinese_woocommerce_get_cart_url' );
		 ...
		 * @since 3.9.0
		 */
		public function aaa_chinese_update_flyout_cart_layout() {
			if ( WC()->cart->is_empty() ) {
				do_action( 'aaa_chinese_empty_cart_before' );
				?>
					<div class="ast-mini-cart-empty">
						<div class="ast-mini-cart-message">
							<p class="woocommerce-mini-cart__empty-message"><?php echo esc_html( apply_filters( 'aaa_chinese_mini_cart_empty_msg', __( 'No products in the cart.', 'astra' ) ) ); ?></p>
						</div>
						<?php do_action( 'aaa_chinese_empty_cart_content' ); ?>
				...
				<?php
				do_action( 'aaa_chinese_empty_cart_after' );
			}
		}
		...
		public function woocommerce_proceed_to_checkout_button_html() {
			$cart_button_text = aaa_chinese_get_option( 'woo-cart-button-text' );
		...
		public function woocommerce_proceed_to_checkout_button() {
			...
			$enable_cart_button_text = aaa_chinese_get_option( 'woo-enable-cart-button-text' );
			$cart_button_text        = aaa_chinese_get_i18n_option( 'woo-cart-button-text', _x( '%astra%', 'WooCommerce Cart: Cart Button Text', 'astra' ) );
		 ...
		 */
		public function aaa_chinese_update_customize_admin_bar_link( $wp_admin_bar ) {
		 ...
		 */
		public static function aaa_chinese_global_btn_woo_comp() {
			$aaa_chinese_settings = aaa_chinese_get_options();
			return apply_filters( 'aaa_chinese_global_btn_woo_comp', isset( $aaa_chinese_settings['global-btn-woo-css'] ) ? false : true );
		}
		 ...
		 */
		public function aaa_chinese_woo_woocommerce_template_product_title( $product_type ) {
		...
		public function woocommerce_product_single_payments() {
			$section_title    = aaa_chinese_get_i18n_option( 'single-product-payment-text', _x( '%astra%', 'WooCommerce Single Product: Payments - Payment Title.', 'astra' ) );
			$if_color_version = aaa_chinese_get_option( 'single-product-payment-icon-color' );
			...
			$payment_list     = aaa_chinese_get_option( 'single-product-payment-list' );
						...
						if ( isset( $single['source'] ) && $single['source'] ) {
							...
							<li class="ast-custom-payment">
								...
								if ( isset( $single['icon'] ) && $single['icon'] ) {
									...
									?>
								<li class="ast-custom-payment">
										<?php echo do_shortcode( AAA_Chinese_Builder_UI_Controller::fetch_svg_icon( $selected_icon, false ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
							</li>
									<?php
								}
							}
		...
		public function single_product_content_structure( $product_type = '' ) {
			...
			/** @psalm-suppress TooManyArguments */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			$single_structure = apply_filters( 'aaa_chinese_woo_single_product_structure', aaa_chinese_get_option( 'single-product-structure' ), $product_type );
			...
			if ( is_array( $single_structure ) && ! empty( $single_structure ) ) {
				 ...
				 */
				$aaa_chinese_addons_condition = aaa_chinese_has_pro_woocommerce_addon();
				...
				foreach ( $single_structure as $value ) {
					...
					switch ( $value ) {
						case 'title':
							 ...
							 */
							do_action( 'aaa_chinese_woo_single_title_before' );
							$this->aaa_chinese_woo_woocommerce_template_product_title( $product_type );
							do_action( 'aaa_chinese_woo_single_title_after' );
							break;
						case 'price':
							/**
							 * Add Product Price on single product page for all products.
							 */
							do_action( 'aaa_chinese_woo_single_price_before' );
							woocommerce_template_single_price();
							do_action( 'aaa_chinese_woo_single_price_after' );
							break;
						case 'ratings':
							/**
							 * Add rating on single product page for all products.
							 */
							do_action( 'aaa_chinese_woo_single_rating_before' );
							woocommerce_template_single_rating();
							do_action( 'aaa_chinese_woo_single_rating_after' );
							break;
						case 'short_desc':
							do_action( 'aaa_chinese_woo_single_short_description_before' );
							woocommerce_template_single_excerpt();
							do_action( 'aaa_chinese_woo_single_short_description_after' );
							break;
						case 'add_cart':
							do_action( 'aaa_chinese_woo_single_add_to_cart_before' );
							woocommerce_template_single_add_to_cart();
							do_action( 'aaa_chinese_woo_single_add_to_cart_after' );
							break;
						case 'summary-extras':
							/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
							if ( $aaa_chinese_addons_condition && is_callable( array( AAA_Ext_WooCommerce_Markup::get_instance(), 'single_product_extras' ) ) ) {
								/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
								do_action( 'aaa_chinese_woo_single_extras_before' );
								AAA_Ext_WooCommerce_Markup::get_instance()->single_product_extras();
								do_action( 'aaa_chinese_woo_single_extras_after' );
							}
							break;
						case 'single-product-payments':
								do_action( 'aaa_chinese_woo_single_product_payments_before' );
								$this->woocommerce_product_single_payments();
								do_action( 'aaa_chinese_woo_single_product_payments_after' );

							break;
						case 'meta':
							do_action( 'aaa_chinese_woo_single_category_before' );
							woocommerce_template_single_meta();
							do_action( 'aaa_chinese_woo_single_category_after' );
							break;
						case 'category':
							do_action( 'aaa_chinese_woo_single_product_category_before' );
							$this->single_product_category();
							do_action( 'aaa_chinese_woo_single_product_category_after' );
							break;
						default:
							break;
					}
				...
				if ( $aaa_chinese_addons_condition && aaa_chinese_get_option( 'accordion-inside-woo-summary' ) && 'accordion' === aaa_chinese_get_option( 'single-product-tabs-layout' ) && aaa_chinese_get_option( 'single-product-tabs-display' ) ) {
					/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
					AAA_Ext_WooCommerce_Markup::get_instance()->woo_product_tabs_layout_output();
				}
			}
		}
		...
		public function single_product_sticky_add_to_cart() {
			...
			if ( is_product() && aaa_chinese_get_option( 'single-product-sticky-add-to-cart' ) ) {
				...
				$sticky_position  = aaa_chinese_get_option( 'single-product-sticky-add-to-cart-position' );
				$add_to_cart_ajax = aaa_chinese_get_option( 'single-product-ajax-add-to-cart' );
				// @codingStandardsIgnoreStart
				/**
				...
				// @codingStandardsIgnoreEnd
					if ( is_customize_preview() ) {
						...
						/** @psalm-suppress TooManyArguments */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
						AAA_Chinese_Builder_UI_Controller::render_customizer_edit_button( 'row-editor-shortcut' );
					} else {
		...
		public function get_quantity_btn_class() {
			$quantity_btn_type = aaa_chinese_get_option( 'cart-plus-minus-button-type', 'normal' );
...
if ( apply_filters( 'aaa_chinese_enable_woocommerce_integration', true ) ) {
	AAA_Chinese_Woocommerce::get_instance();
}
