 ...
 * @return string
 */
function aaa_chinese_get_breadcrumb( $echo = true ) {

	if ( ! $echo ) {
		return '<div class="ast-breadcrumbs-wrapper">
			<div class="ast-breadcrumbs-inner">' .
				aaa_chinese_get_selected_breadcrumb( $echo ) .
			'</div>
		</div>';
	}
	...
	<div class="ast-breadcrumbs-wrapper">
		<div class="ast-breadcrumbs-inner">
			<?php aaa_chinese_get_selected_breadcrumb( $echo ); ?>
		</div>
	</div>
	<?php
}
 ...
 */
function aaa_chinese_get_selected_breadcrumb( $echo = true ) {
	...
	if ( function_exists( 'rank_math_the_breadcrumbs' ) && $breadcrumb_source && 'rank-math' === $breadcrumb_source ) {
		...
		// Load default AAA Chinese breadcrumb if none selected.
		return aaa_chinese_get_breadcrumb_trail( $echo );
	}
}

/**
 * Deprecating aaa_chinese_breadcrumb_trail function.
 *
 * @since 1.8.1
 * @deprecated 1.8.1 Use aaa_chinese_get_breadcrumb()
 * @param array  $args List of args.
 * @see aaa_chinese_breadcrumb_trail()
 *
 * @return string new breadcrumb function.
 */
function aaa_chinese_breadcrumb_trail( $args = array() ) {
	_deprecated_function( __FUNCTION__, '1.8.1', 'aaa_chinese_get_breadcrumb()' );
 ...
 * @return string Selected Breadcrumb.
 */
function aaa_chinese_get_breadcrumb_trail( $echo = true ) {
	...
	$args = apply_filters( 'aaa_chinese_breadcrumb_trail_args', $defaults );

	$breadcrumb = apply_filters( 'aaa_chinese_breadcrumb_trail_object', null, $args );

	if ( ! is_object( $breadcrumb ) ) {
		$breadcrumb = new AAA_Chinese_Breadcrumb_Trail( $args );
	}
 ...
 * @since  0.6.0
 */
class AAA_Chinese_Breadcrumb_Trail {
	/**
	...
	public function __construct( $args = array() ) {
		...
		$this->args = apply_filters( 'aaa_chinese_breadcrumb_trail_args', wp_parse_args( $args, $defaults ) );
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			if ( $this->args['schema'] ) {
				...
				$breadcrumb .= sprintf( '<meta content="%1$d" %2$s />', absint( $item_count ), aaa_chinese_attr(
				...
				$breadcrumb .= '<meta ' . aaa_chinese_attr(
		...
		$breadcrumb = apply_filters( 'aaa_chinese_breadcrumb_trail', $breadcrumb, $this->args );
	...
	protected function set_labels() {
		...
		$this->labels = apply_filters( 'aaa_chinese_breadcrumb_trail_labels', wp_parse_args( $this->args['labels'], $defaults ) );
	}
	...
	protected function set_post_taxonomy() {
		...
		$this->post_taxonomy = apply_filters( 'aaa_chinese_breadcrumb_trail_post_taxonomy', wp_parse_args( $this->args['post_taxonomy'], $defaults ) );
	}
	...
	protected function add_items() {
		...
		$this->items = array_unique( apply_filters( 'aaa_chinese_breadcrumb_trail_items', $this->items, $this->args ) );
	}
	...
	protected function get_translated_term_name( $term ) {
		...
		$translated_name = apply_filters( 'aaa_chinese_breadcrumb_translated_term_name', $translated_name, $term );
