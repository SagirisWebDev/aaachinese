...
if ( ! class_exists( 'AAA_Chinese_ExtTransparenty_Header_Markup' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class AAA_Chinese_Ext_Transparent_Header_Markup {
		/**
		 * Member Variable
		 *
		 * @var object instance
		...
		public function __construct() {
			...
			/* Fixed header markup */
			add_action( 'aaa_chinese_header', array( $this, 'transparent_header_logo' ), 1 );

			/**
			 * Metabox setup
			 */
			add_filter( 'aaa_chinese_meta_box_options', array( $this, 'add_options' ) );
			add_action( 'aaa_chinese_meta_box_markup_after', array( $this, 'add_options_markup' ) );

			add_action( 'aaa_chinese_customizer_save', array( $this, 'customizer_save' ) );
		}
		...
		public function add_body_class( $classes ) {
			...
			$inherit_desk_logo              = aaa_chinese_get_option( 'different-transparent-logo', false );
			$transparent_header_logo        = aaa_chinese_get_option( 'transparent-header-logo', true );
			$transparent_header_logo_retina = aaa_chinese_get_option( 'transparent-header-retina-logo', true );
		...
		/**
		 * AAA Chinese check if transparent header is enabled.
		 *
		...
		public static function is_transparent_header() {
			...
			// Transparent Header.
			$enable_trans_header = aaa_chinese_get_option( 'transparent-header-enable' );
			$trans_meta_option   = aaa_chinese_get_option_meta( 'theme-transparent-header-meta', 'default' );
			...
			if ( $enable_trans_header ) {
				...
				if ( is_404() && '1' == aaa_chinese_get_option( 'transparent-header-disable-404-page', aaa_chinese_get_option( 'transparent-header-disable-archive' ) ) ) {
				...
				if ( is_search() && '1' == aaa_chinese_get_option( 'transparent-header-disable-search-page', aaa_chinese_get_option( 'transparent-header-disable-archive' ) ) ) {
					$enable_trans_header = false;
				}
				...
				if ( is_archive() && '1' == aaa_chinese_get_option( 'transparent-header-disable-archive-pages', aaa_chinese_get_option( 'transparent-header-disable-archive' ) ) ) {
					$enable_trans_header = false;
				}

				if ( is_home() && '1' == aaa_chinese_get_option( 'transparent-header-disable-index' ) && ( 'posts' !== get_option( 'show_on_front' ) ) ) {
					$enable_trans_header = false;
				}

				if ( is_front_page() && 'posts' === get_option( 'show_on_front' ) && '1' == aaa_chinese_get_option( 'transparent-header-disable-latest-posts-index' ) ) {
					$enable_trans_header = false;
				}

				if ( is_page() && '1' == aaa_chinese_get_option( 'transparent-header-disable-page' ) ) {
					$enable_trans_header = false;
				}

				if ( is_single() && '1' == aaa_chinese_get_option( 'transparent-header-disable-posts' ) ) {
					$enable_trans_header = false;
				}
			}

			if ( class_exists( 'AAA_Chinese_Woocommerce' ) ) {
				if ( is_product() && '1' == aaa_chinese_get_option( 'transparent-header-disable-woo-products' ) ) {
					$enable_trans_header = false;
				}
			}
			...
			return apply_filters( 'aaa_chinese_is_transparent_header', $enable_trans_header );
		}
		...
		public function transparent_header_logo() {
			...
			if ( self::is_transparent_header() && '1' == $inherit_desk_logo && '' !== $transparent_header_logo ) {
				...
				add_filter( 'aaa_chinese_has_custom_logo', '__return_true' );
		...
		public function replace_trans_header_attr( $attr, $attachment, $size ) {
			...
			if ( $custom_logo_id == $attachment->ID ) {
				...
				$diff_retina_logo = aaa_chinese_get_option( 'different-transparent-retina-logo' );

				if ( '1' == $diff_retina_logo ) {

					$retina_logo = aaa_chinese_get_option( 'transparent-header-retina-logo' );

					$attr['srcset'] = '';

					if ( apply_filters( 'aaa_chinese_transparent_header_retina', true ) && '' !== $retina_logo ) {
		...
		public function add_options_markup( $meta ) {
			...
			$show_meta_field   = ! aaa_chinese_check_is_bb_themer_layout();
			?>
		...
		public function customizer_save() {
			...
			if ( '' !== $trans_logo ) {
				...
				AAA_Chinese_Customizer::generate_logo_by_width( $trans_logo_id );
		...
		public function transparent_logo_image_sizes( $sizes, $metadata ) {
			...
			$logo_width = aaa_chinese_get_option( 'transparent-header-logo-width' );
 ...
 */
AAA_Chinese_Ext_Transparent_Header_Markup::get_instance();
