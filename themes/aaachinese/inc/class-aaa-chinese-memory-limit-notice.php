...
/**
 * Class AAA_Chinese_Memory_Limit_Notice
 */
class AAA_Chinese_Memory_Limit_Notice {
	/**
	...
	/**
	 * Add memory notice using AAA_Chinese_Notices system
	 */
	public function add_memory_notice() {
		...
		// Add notice using AAA_Chinese_Notices system.
	...
	/**
	 * Register memory notice with AAA_Chinese_Notices system
	 *
	...
	private function display_memory_notice( $notice_data ) {
		...
		$message .= '<p><a href="https://wpastra.com/docs/system-requirement-for-aaa-chinese-theme/" target="_blank" rel="noopener">';
		...
		AAA_Chinese_Notices::add_notice( $notice_args );
	}
	...
	public function add_site_health_tests( $tests ) {
		$tests['direct']['aaa_chinese_memory_usage'] = array(
			'label' => __( 'AAA Chinese Theme Memory Usage', 'astra' ),
	...
	public function site_health_memory_test() {
		...
		return array(
			...
			'status'      => $status,
			'badge'       => array(
				'label' => __( 'AAA Chinese Theme', 'astra' ),
				'color' => $badge_color,
			),
			'description' => $description,
			'actions'     => $actions,
			'test'        => 'aaa_chinese_memory_usage',
		);
	}
	...
	private function get_site_health_description( $memory_percentage, $memory_limit_formatted, $memory_usage_formatted, $memory_remaining_formatted, $percentage_formatted ) {
		...
		if ( $memory_percentage >= $this->warning_threshold ) {
			...
			$description .= '</p><p>';
			$description .= __( 'Your memory usage is within acceptable limits. The AAA Chinese theme and your plugins have sufficient memory to operate properly.', 'astra' );
		}
	...
	private function get_site_health_actions( $memory_percentage ) {
		...
		if ( $memory_percentage >= $this->warning_threshold ) {
			...
			$actions .= sprintf(
				...
				'https://wpastra.com/docs/system-requirement-for-aaa-chinese-theme/'
			);
			$actions .= '</p>';
		} else {
...
new AAA_Chinese_Memory_Limit_Notice();
