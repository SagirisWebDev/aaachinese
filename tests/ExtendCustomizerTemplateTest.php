...
class ExtendCustomizerTemplateTest extends TestCase {
    ...
    private function templatePath(): string {
        return DYNAMO_PATH . 'dynamo-extend-customizer.php';
    }
    ...
    public function test_examples_are_commented_out(): void {
        $lines = preg_split('/\R/', $this->templateBody());
        foreach ($lines as $n => $line) {
            if (preg_match('/^\s*dynamo_config_customizer\s*\(/', $line)) {
                $this->fail(
                    "Line " . ($n + 1) . " contains a live `dynamo_config_customizer(` call: '"
    ...
    public function test_including_file_registers_no_bindings(): void {
        Dynamo_Binding_Registry::reset_instance();
        require $this->templatePath();
        $this->assertSame(
            [],
            Dynamo_Binding_Registry::instance()->all(),
            'Shipped template must register zero Bindings.'
        );
    }
}
